...
void myyyerror(const char *s, ...) { printf("error: %s\n", s); }

inline void insertNodePointer(Node *a, vector<Node *> *allnode) {
  if (NULL != allnode) allnode->push_back(a);
}
...
struct Node *newExpr(nodetype t, char *d, vector<Node *> *allnode) {
  ...
  ////2014-4-2---将这部分工作放在词法识别阶段进行，见sql.l
  //	{
...
struct Node *newColumn(nodetype t, char *parameter1, char *parameter2,
                       Node *next, vector<Node *> *allnode) {
...
struct Node *newExprCal(nodetype type, char *sign, char *parameter, int cmp,
                        Node *lnext, Node *rnext, vector<Node *> *allnode) {
...
struct Node *newJoin(nodetype type, int jointype, Node *lnext, Node *rnext,
                     Node *condition, vector<Node *> *allnode) {
...
struct Node *newQueryStmt(nodetype t, char *querystring, int select_opts,
                          Node *select_list, Node *from_list, Node *where_list,
                          Node *groupby_list, Node *having_list,
                          Node *orderby_list, Node *limit_list, Node *into_list,
                          vector<Node *> *allnode) {
...
struct Node *newSelectList(nodetype type, int isall, Node *args, Node *next,
                           vector<Node *> *allnode) {
  struct Select_list *a =
      (struct Select_list *)malloc(sizeof(struct Select_list));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newSelectExpr(nodetype type, char *ascolname, Node *colname,
                           vector<Node *> *allnode) {
  struct Select_expr *a =
      (struct Select_expr *)malloc(sizeof(struct Select_expr));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newGroupbyList(nodetype type, char *groupbystring, Node *next,
                            int with_rollup, vector<Node *> *allnode) {
...
struct Node *newOrderbyExpr(nodetype type, Node *args, char *sorttype,
                            Node *next, vector<Node *> *allnode) {
  struct Orderby_expr *a =
      (struct Orderby_expr *)malloc(sizeof(struct Orderby_expr));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
Node *newInsertStmt(int insert_opt, char *tablename, Node *col_list,
                    ...
                    Node *insert_assign_list_from_set, Node *select_stmt,
                    vector<Node *> *allnode) {
...
// 2014-4-17---add---by Yu
Node *newInsertAssignList(char *col_name, int value_type, Node *expr,
                          Node *next, vector<Node *> *allnode) {
  Insert_assign_list *a =
      (Insert_assign_list *)malloc(sizeof(Insert_assign_list));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
/*** 		create 语句			***/
struct Node *newCreateDatabaseStmt(nodetype type, int create_type, int check,
                                   char *name, vector<Node *> *allnode) {
  struct Create_database_stmt *a =
      (Create_database_stmt *)malloc(sizeof(Create_database_stmt));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newCreateTableStmt(nodetype type, int create_type, int check,
                                char *name1, char *name2, Node *list,
                                Node *select_stmt, vector<Node *> *allnode) {
  struct Create_table_stmt *a =
      (struct Create_table_stmt *)malloc(sizeof(struct Create_table_stmt));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newCreateColList(nodetype type, Node *data, Node *next,
                              vector<Node *> *allnode) {
  struct Create_col_list *a =
      (struct Create_col_list *)malloc(sizeof(struct Create_col_list));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newCreateDef(
    nodetype type, int deftype, char *name, Node *datatype, Node *col_atts,
    Node *col_list, vector<Node *> *allnode) {  // 2-18---增加name属性---by余楷
...
struct Node *newColumnAtts(nodetype type, int datatype, int num1, double num2,
                           char *s, Node *col_list, vector<Node *> *allnode) {
...
struct Node *newCreateSelectStmt(nodetype type, int ignore_replace,
                                 int temporary, Node *select_stmt,
                                 vector<Node *> *allnode) {
  Create_select_stmt *a =
      (Create_select_stmt *)malloc(sizeof(Create_select_stmt));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newCreateProjectionStmt(nodetype type, char *tablename,
                                     Node *column_list, int partition_num,
                                     char *partition_attribute_name,
                                     vector<Node *> *allnode) {
  Create_projection_stmt *a =
      (Create_projection_stmt *)malloc(sizeof(Create_projection_stmt));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newTruncateStmt(nodetype type, char *name,
                             vector<Node *> *allnode) {
  struct Truncate_stmt *a =
      (struct Truncate_stmt *)malloc(sizeof(struct Truncate_stmt));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newAlterDatabaseStmt(nodetype type, int createtype, char *name,
                                  Node *opt, vector<Node *> *allnode)

{
  Alterdatabase_stmt *a =
      (Alterdatabase_stmt *)malloc(sizeof(Alterdatabase_stmt));
  if (!a) {
    myyyerror("out of space!");
    assert(false);
  }
...
struct Node *newAlterTableStmt(nodetype type, int isignore, char *name,
                               Node *parameter, vector<Node *> *allnode) {
...
// 2014-3-24---增加---by Yu
struct Node *newCreateIndex(nodetype type, int index_att, char *index_name,
                            int index_type, char *table_name,
                            Node *index_col_name, vector<Node *> *allnode) {
...
// 2014-3-24---增加---by Yu
struct Node *newIndexColList(nodetype type, char *name, Node *length,
                             int asc_desc, Node *next,
                             vector<Node *> *allnode) {
...
// 2014-3-24---增加---by Yu
struct Node *newDropDatabase(nodetype type, int drop_type, int check,
                             char *name, vector<Node *> *allnode) {
...
// 2014-3-24---增加---by Yu
struct Node *newDropTable(nodetype type, int is_temp, int is_check, int opt_rc,
                          Node *table_list, vector<Node *> *allnode) {
...
// 2014-3-27---增加---by Yu
struct Node *newLoadTable(nodetype type, char *table_name, Node *path,
                          char *column_separator, char *tuple_separator,
                          double sample, int mode, vector<Node *> *allnode) {
...
Node *newShowStmt(int show_type, bool full, char *database_name,
                  char *like_string,
                  vector<Node *> *allnode)  // 2014-5-4---add ---by Yu
{
