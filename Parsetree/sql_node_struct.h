...
/*
 * enum data_type{
 * 	t_smallInt,t_int,t_u_long,t_float,t_double,t_string, t_date, t_time,
 * t_datetime, t_decimal, t_boolean, t_u_smallInt};
 * from data_type.h
 */
...
struct Expr_func  //函数表达式，将is null/exist等判断抽象成函数
  ...
  Node *args;
  Node *parameter1,
      *parameter2;  //函数中的参数列表，处理between...and.../case...when...then...end等
  Node *next;
};
...
struct Insert_stmt {
  ...
  Node *insert_assign_list_from_set;  // 2014-4-17---not used in this version
                                      // ---by Yu
...
struct Node *newStmt(
    nodetype t, Node *list, Node *newNode,
    vector<Node *> *allnode);  // 2014-3-4---增加新建语句列表函数---by余楷
...
struct Node *newIndexColList(nodetype type, char *name, Node *length,
                             int asc_desc, Node *next, vector<Node *> *allnode);
