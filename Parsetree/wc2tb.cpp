...
int judgepos(
    struct Node *args,
    set<string>
        st)  //判断fromlist当前节点的值（args）是否是第一次涉及到st中的表，完成定位
{
  switch (args->type) {
    case t_table: {
...
void setwcposition(struct Node *wccur, struct Node *flcur,
                   set<string> &st)  //在fromlist中定位并把wcexpr放入
{
  switch (flcur->type) {
    case t_from_list: {
      ...
      if (judgeresult == 0) {
        if (node->next) setwcposition(wccur, node->next, st);
      } else {
        if (judgeresult == 1 &&
            st.size() == 1)  //=1定位在单个table上，>1定位在fromlist上
        {
    ...
    //		case
    // t_join://应该把join看作一个整体，然后把where中的条件放在这里
    //		{
...
void getwctable(struct Node *cur,
                set<string> &st)  // cur=wcexpr,获得wcexpr中涉及的表并放入set中
{
  //	cout<<"wc "<<cur->type<<"  "<<(st.size())<<endl;
  if (cur->type == t_name)  // 2014-4-16---modify because Columns struct has the
                            // t_name for NAME, while Expr struct has the t_name
                            // for USERVAR---by Yu
  {
    //		Expr *node=(Expr *)(cur);
    //		if(node->data == NULL)
    ...
    st.insert(string(node->parameter2));  //应该获得的是表名？？？	<<<
                                          ////
    // 2014-4-16---Now it is the name of table
    //---by Yu
  } else if (cur->type == t_name_name) {
    Columns *node = (Columns *)(cur);
    st.insert(
        node->parameter1);  //为了保证测试，测试的时候用A.a的格式，就可以保证获得表名
    ...
    if (node->next != NULL) getwctable(node->next, st);
  } else {
    SQLParse_log("wc2tb can't konw the type %d\n", cur->type);
  }
  return;
}
void solvewc(struct Node *wcexpr,
             struct Node *fromlist)  //分两个过程，1.获得wcexpr中的表
// 2.在fromlist中寻找相应的位置并加入wcexpr
{
  set<string> st;
  st.clear();
...
void departwc(
    struct Node *wherecondition,
    struct Node *
        fromlist)  //对wherecondition的A范式分解，对每一个wcexpr调用solvewc把wherecondition放在fromlist相应的位置上
{
...
int setocposition(
    struct Node *wccur, struct Node *flcur, set<string> &st,
    vector<Node *> &jcondition)  //在fromlist中定位并把wcexpr放入oc=on condition
{
  int judgeresult = judgepos(flcur, st);
  if (judgeresult == 0) return 0;
  switch (flcur->type) {
    case t_table: {
      if (st.size() == 1) {
        Table *node = (Table *)flcur;
...
int set_join_condition_on_joinnode(Join *joinnode, Node *condition,
...
int depart_join_condition(Join *joinnode, Node *condition,
                          vector<Node *> &jcondition) {
  Node *cur = condition;
  if (cur->type == t_expr_cal) {
    struct Expr_cal *node = (struct Expr_cal *)cur;
