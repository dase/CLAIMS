...
static int getjoinpairlist(Node *wcexpr,vector<LogicalEqualJoin::JoinPair> &join_pair_list,LogicalOperator *filter_1,LogicalOperator * filter_2)
{
	switch(wcexpr->type)
	{
...
//						join_pair_list.push_back(EqualJoin::JoinPair(filter_1->getDataflow().getAttribute(lnode->parameter2),filter_2->getDataflow().getAttribute(rnode->parameter2)));
					//	printf("left %s.%s   right %s.%s\n",lnode->parameter1,lnode->parameter2,rnode->parameter1,rnode->parameter2);
						...
						Attribute la=filter_1->GetPlanContext().GetAttribute(lnode->parameter2);
						Attribute ra=filter_2->GetPlanContext().GetAttribute(rnode->parameter2);
			...
			if(strcmp(node->sign,"INS")==0)
			{
				if(node->rnext->type==t_query_stmt)
				{
					...
					for(lp=node->lnext,sp=((Query_stmt *)node->rnext)->select_list;lp!=NULL;)//3.1获得equaljoin的左右属性
					{
						...
						join_pair_list.push_back(LogicalEqualJoin::JoinPair(input->GetPlanContext().GetAttribute(lcol->parameter2),sublogicalplan->GetPlanContext().GetAttribute(rcol->parameter2)));
						lp=lpexpr->next;
						sp=spexpr->next;
					}
					LogicalOperator* join_logicalplan=new LogicalEqualJoin(join_pair_list,input,aggrection_sublogicalplan);
...
//				tablescan=new LogicalScan(Environment::getInstance()->getCatalog()->getTable(std::string(node->tablename))->get_table_id());//
			...
			else//need to modify the output_schema_attrname from the subquery to the form of subquery's alias.attrname
			{
				...
				vector<Attribute>output_attribute=tablescan->GetPlanContext().attribute_list_;
	...
	switch(selectlist->type)
	{
		...
		case t_expr_func:
		{
			...
			else if(strcmp(funcnode->funname,"FCOUNT")==0)
			{
				...
				{
					aggregation_attributes.push_back(input->GetPlanContext().GetAttribute(get_expr_str(funcnode->parameter1)));
				}
			}
...
static void get_group_by_attributes(Node *groupby_node,vector<Attribute> &group_by_attributes,LogicalOperator *input)
{
	...
	switch(groupby_node->type)
	{
		...
		case t_column:
		{
			group_by_attributes.push_back(input->GetPlanContext().GetAttribute(get_expr_str(groupby_node)));
		}break;
		case t_expr_cal:
		{
			group_by_attributes.push_back(input->GetPlanContext().GetAttribute(get_expr_str(groupby_node)));
