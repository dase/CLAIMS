...
//					printf("++++++++++++++++Q1: execution time: %4.4f second.++++++++++++++\n",getSecond(start));
			...
			case t_insert_stmt:	// 2014-4-19---add---by Yu	// 2014-5-1---modify---by Yu
			{
				...
				if(table == NULL)
				{
					error_msg="The table "+ table_name+" does not exist during inserting!";
					result_flag=false;
					break;
				}
				...
				if (insert_value_list == NULL) {
					error_msg="No value during inserting!";
					result_flag=false;
					break;
				}
				...
				while(insert_value_list)
				{
					...
					if (is_all_col)	// insert all columns
					{
						...
						for(unsigned int position = 1; position < table->getNumberOfAttribute(); position++)
						{
							...
							{
								error_msg="Value count is too few during inserting";
								result_flag=false;
								return;
								is_correct = false;
								break;
						...
						if (insert_value)
						{
							error_msg="Value count is too many during inserting";
							result_flag=false;
							return;
							is_correct = false;
							break;
					...
					else	//insert part of columns
					{
						...
						{
							error_msg="Column count doesn't match value count during inserting";
							result_flag=false;
							return;
							is_correct = false;
							break;
						...
						{
							error_msg="Some columns don't exist during inserting";
							result_flag=false;
							is_correct = false;
							break;
						}
