...
static int parser_test(){
	...
	//		catalog->add_table(table_1);
			...
			catalog->add_table(table_1);// 建表之后放入
	...
	//		table_2->createHashPartitionedProjection(sb_proj0_index,"order_no",4);	//G0
			...
			for(unsigned int i=0;i<table_1->getProjectoin(0)->getPartitioner()->getNumberOfPartitions();i++){
			...
			///////////////////ONE MONTH/////////////////////////////////////////////////////////////
			//CJ NNNNNNNNNNNOOOOOOOOOOOOOOOOOOOOOO
			// 4 partition
...
//				printf("query optimization time :%5.5f\n",getMilliSecond(timer_start));
			...
			while(c==1){
				timer_start=curtick();
				IteratorExecutorMaster::getInstance()->ExecuteBlockStreamIteratorsOnSite(please,"127.0.0.1");//						executable_query_plan->open();//			while(executable_query_plan->next(0));
