...
static void query_1() {
  ...
  LogicalOperator* root =
      new LogicalQueryPlanRoot(0, filter, LogicalQueryPlanRoot::PERFORMANCE);
...
static void query_2() {
  ...
  r_n_join_condition.push_back(
      EqualJoin::JoinPair(region->getAttribute("REGION.R_REGIONKEY"),
  ...
  n_s_join_condition.push_back(
      EqualJoin::JoinPair(nation->getAttribute("NATION.N_NATIONKEY"),
                          supplier->getAttribute("SUPPLIER.S_NATIONKEY")));
  LogicalOperator* n_s_join =
      new EqualJoin(n_s_join_condition, r_n_join, s_scan);

  std::vector<EqualJoin::JoinPair> s_ps_join_condition;
  s_ps_join_condition.push_back(
      EqualJoin::JoinPair(supplier->getAttribute("SUPPLIER.S_SUPPKEY"),
                          partsupp->getAttribute("PARTSUPP.PS_SUPPKEY")));
  LogicalOperator* s_ps_join =
      new EqualJoin(s_ps_join_condition, n_s_join, ps_scan);

  std::vector<EqualJoin::JoinPair> p_ps_join_condition;
  p_ps_join_condition.push_back(
      EqualJoin::JoinPair(partsupp->getAttribute("PARTSUPP.PS_PARTKEY"),
                          part->getAttribute("PART.P_PARTKEY")));
  LogicalOperator* s_ps_n_join =
      new EqualJoin(p_ps_join_condition, s_ps_join, p_scan);

  std::vector<Attribute> aggregation_attributes;
  aggregation_attributes.push_back(
      s_ps_join->getDataflow().getAttribute("PARTSUPP.PS_SUPPLYCOST"));
  std::vector<BlockStreamAggregationIterator::State::aggregation>
      aggregation_function;
  aggregation_function.push_back(BlockStreamAggregationIterator::State::min);
  LogicalOperator* aggregation =
      new Aggregation(std::vector<Attribute>(), aggregation_attributes,
                      aggregation_function, s_ps_n_join);

  LogicalOperator* root = new LogicalQueryPlanRoot(
      0, s_ps_n_join, LogicalQueryPlanRoot::RESULTCOLLECTOR);

  BlockStreamIteratorBase* sub_physical_iterator_tree =
      root->getIteratorTree(64 * 1024);
  ...
  p_ps_farther_join_condition.push_back(
      EqualJoin::JoinPair(part->getAttribute("PART.P_PARTKEY"),
                          partsupp->getAttribute("PARTSUPP.PS_PARTKEY")));
  LogicalOperator* p_ps_farther_join =
      new EqualJoin(p_ps_farther_join_condition, p_scan, ps_filter);
  ...
  LogicalOperator* r_filter_father =
      new Filter(r_filter_father_condition, r_scan);
  ...
  LogicalOperator* r_n_farther_join =
      new EqualJoin(r_n_farther_join_condition, r_scan, n_scan);

  std::vector<EqualJoin::JoinPair> r_n_s_farther_join_condition;
  r_n_s_farther_join_condition.push_back(
      EqualJoin::JoinPair(nation->getAttribute("NATION.N_NATIONKEY"),
                          supplier->getAttribute("SUPPLIER.S_NATIONKEY")));
  LogicalOperator* r_n_s_farther_join =
      new EqualJoin(r_n_s_farther_join_condition, r_n_farther_join, s_scan);

  std::vector<EqualJoin::JoinPair> r_n_s_p_ps_farther_join_condition;
  r_n_s_p_ps_farther_join_condition.push_back(
      EqualJoin::JoinPair(supplier->getAttribute("SUPPLIER.S_SUPPKEY"),
                          partsupp->getAttribute("PARTSUPP.PS_SUPPKEY")));
  LogicalOperator* r_n_s_p_ps_farther_join = new EqualJoin(
  ...
  LogicalOperator* root_father = new LogicalQueryPlanRoot(
      0, r_n_s_p_ps_farther_join, LogicalQueryPlanRoot::PERFORMANCE);
  BlockStreamIteratorBase* final_physical_iterator_tree =
      root_father->getIteratorTree(64 * 1024);
  //
...
static void query_3() {
  ...
  c_o_join_condition.push_back(
      EqualJoin::JoinPair(customer->getAttribute("CUSTOMER.C_CUSTKEY"),
                          orders->getAttribute("ORDERS.O_CUSTKEY")));
  LogicalOperator* c_o_join =
      new EqualJoin(c_o_join_condition, c_filter, o_filter);
  ...
  c_o_l_join_condition.push_back(
      EqualJoin::JoinPair(orders->getAttribute("ORDERS.O_ORDERKEY"),
                          lineitem->getAttribute("LINEITEM.L_ORDERKEY")));
  LogicalOperator* c_o_l_join =
      new EqualJoin(c_o_l_join_condition, c_o_join, l_filter);

  std::vector<Attribute> groupby_attributes;
  groupby_attributes.push_back(
      c_o_l_join->getDataflow().getAttribute("LINEITEM.L_ORDERKEY"));
  groupby_attributes.push_back(
      c_o_l_join->getDataflow().getAttribute("ORDERS.O_ORDERDATE"));
  groupby_attributes.push_back(
      c_o_l_join->getDataflow().getAttribute("ORDERS.O_SHIPPRIORITY"));
  std::vector<Attribute> aggregation_attributes;
  aggregation_attributes.push_back(
      c_o_l_join->getDataflow().getAttribute("LINEITEM.L_EXTENDEDPRICE"));
  aggregation_attributes.push_back(
      c_o_l_join->getDataflow().getAttribute("LINEITEM.L_DISCOUNT"));
  std::vector<BlockStreamAggregationIterator::State::aggregation>
      aggregation_function;
  aggregation_function.push_back(BlockStreamAggregationIterator::State::sum);
  aggregation_function.push_back(BlockStreamAggregationIterator::State::sum);
  LogicalOperator* aggregation =
      new Aggregation(groupby_attributes, aggregation_attributes,
                      aggregation_function, c_o_l_join);

  LogicalOperator* root = new LogicalQueryPlanRoot(
      0, aggregation, LogicalQueryPlanRoot::PERFORMANCE);
  BlockStreamIteratorBase* final_physical_iterator_tree =
      root->getIteratorTree(64 * 1024);
