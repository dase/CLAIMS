...
static int test_scan() {
  ...
  LogicalOperator* cj_join_key_scan =
      new LogicalScan(table_1->getProjectoin(0));
  ...
  filter_condition_1.add(table_1->getAttribute(1), AttributeComparator::EQ,
                         std::string("20101008"));
  ...
  LogicalOperator* filter_1 =
      new LogicalFilter(filter_condition_1, cj_join_key_scan);

  const NodeID collector_node_id = 0;
  LogicalOperator* root = new LogicalQueryPlanRoot(
      collector_node_id, filter_1, LogicalQueryPlanRoot::kPerformance);
...
static int test_scan_filter_high_selectivity() {
  ...
  LogicalFilter::Condition filter_condition_1;
  const int order_type = 1;
  filter_condition_1.add(table_1->getAttribute(5), AttributeComparator::EQ,
                         std::string("1"));
  const int trade_date = 20101008;
  filter_condition_1.add(table_1->getAttribute(1), AttributeComparator::GEQ,
                         std::string("20101008"));
  const int sec_code = 600036;
  filter_condition_1.add(table_1->getAttribute(3), AttributeComparator::GEQ,
                         std::string("600036"));
  LogicalOperator* filter_1 =
      new LogicalFilter(filter_condition_1, cj_join_key_scan);
  ...
  IteratorExecutorSlave::executePhysicalQueryPlan(
      PhysicalQueryPlan(executable_query_plan));
  //	executable_query_plan->open();
  //	while(executable_query_plan->next(0));
  //	executable_query_plan->close();
  ...
  const unsigned long int number_of_tuples =
      executable_query_plan->GetNumberOfTuples();
  if (!print_test_name_result(number_of_tuples == 3651348,
...
static int test_scan_filter_low_selectivity() {
  ...
  //	QColcumns *qcol=new
  // QColcumns("cj","row_id",t_u_long,"cj.row_id");//table_name,column_name,actual_type,alias
  //	QExpr *qexpr=new QExpr("1",t_u_long,"1");//data(string),type,alias
  //	QExpr_binary *qcalnode=new
  // QExpr_binary(qcol,qexpr,t_u_long,oper_equal,t_qexpr_cmp,"cj.row_id=1");
  //	vector<QNode *>qual;
  ...
  filter_condition_1.add(table_1->getAttribute(1), AttributeComparator::GEQ,
                         std::string("20101008"));
  const int sec_code = 600036;
  filter_condition_1.add(table_1->getAttribute(3), AttributeComparator::EQ,
                         std::string("600036"));
  LogicalOperator* filter_1 =
      new LogicalFilter(filter_condition_1, cj_join_key_scan);
  ...
  const unsigned long int number_of_tuples =
      executable_query_plan->GetNumberOfTuples();
  if (!print_test_name_result(number_of_tuples == 26695,
...
static int test_scan_Aggregation_small_Groups() {
  ...
  LogicalOperator* aggregation =
      new LogicalAggregation(group_by_attributes, aggregation_attributes,
                             aggregation_function, cj_join_key_scan);
  ...
  if (!print_test_name_result(number_of_tuples == 1022,
                              "Scan Aggregation small groups")) {
...
static int test_scan_Aggregation_large_Groups() {
  ...
  const unsigned long int number_of_tuples = result_set->getNumberOftuples();
  if (!print_test_name_result(number_of_tuples == 3966020,
                              "Scan Aggregation large groups")) {
    printf("\tExpected:870 actual: %d\n", number_of_tuples);
  }
  delete executable_query_plan;
  delete root;
...
static int test_scan_filter_Aggregation() {
  ...
  LogicalOperator* aggregation =
      new LogicalAggregation(group_by_attributes, aggregation_attributes,
                             aggregation_function, filter_1);

  const NodeID collector_node_id = 0;
  LogicalOperator* root = new LogicalQueryPlanRoot(
      collector_node_id, aggregation, LogicalQueryPlanRoot::kPerformance);
  ...
  const unsigned long int number_of_tuples =
      executable_query_plan->GetNumberOfTuples();
  if (!print_test_name_result(number_of_tuples == 870,
...
static int test_scan_filter_Scalar_Aggregation() {
  ...
  const unsigned long int number_of_tuples =
      *(unsigned long*)b_it->currentTuple();
  delete b_it;

  if (!print_test_name_result(number_of_tuples == 3651348,
                              "Filtered Scalar Aggregation")) {
    printf("\tExpected:3651348 actual: %d\n", number_of_tuples);
  }
...
static int test_no_repartition_filtered_join() {
  ...
  LogicalOperator* cj_join_key_scan =
      new LogicalScan(table_1->getProjectoin(0));
  LogicalOperator* sb_join_key_scan =
      new LogicalScan(table_2->getProjectoin(0));
  ...
  filter_condition_2.add(table_2->getAttribute(4), AttributeComparator::EQ,
                         std::string("1"));
  const int entry_date = 20101008;
  filter_condition_2.add(table_2->getAttribute(2), AttributeComparator::GEQ,
                         std::string("20101008"));
  const int sec_code_ = 600036;
  filter_condition_2.add(table_2->getAttribute(3), AttributeComparator::EQ,
                         std::string("600036"));
  LogicalOperator* filter_2 =
      new LogicalFilter(filter_condition_2, sb_join_key_scan);

  std::vector<LogicalEqualJoin::JoinPair> sb_cj_join_pair_list;
  sb_cj_join_pair_list.push_back(LogicalEqualJoin::JoinPair(
      table_1->getAttribute("row_id"), table_2->getAttribute("row_id")));
  ...
  LogicalOperator* sb_cj_join =
      new LogicalEqualJoin(sb_cj_join_pair_list, filter_1, filter_2);

  const NodeID collector_node_id = 0;
  LogicalOperator* root = new LogicalQueryPlanRoot(
      collector_node_id, sb_cj_join, LogicalQueryPlanRoot::kResultCollector);
  ...
  if (!print_test_name_result(number_of_tuples == 15,
                              "Filtered no_repartition Join")) {
...
static int test_complete_repartition_filtered_join() {
  ...
  std::vector<LogicalEqualJoin::JoinPair> sb_cj_join_pair_list;
  sb_cj_join_pair_list.push_back(LogicalEqualJoin::JoinPair(
      table_1->getAttribute("order_no"), table_2->getAttribute("order_no")));
  sb_cj_join_pair_list.push_back(
      LogicalEqualJoin::JoinPair(table_1->getAttribute("trade_date"),
                                 table_2->getAttribute("entry_date")));
  sb_cj_join_pair_list.push_back(LogicalEqualJoin::JoinPair(
      table_1->getAttribute("trade_dir"), table_2->getAttribute("entry_dir")));
  LogicalOperator* sb_cj_join =
      new LogicalEqualJoin(sb_cj_join_pair_list, filter_1, filter_2);
  ...
  if (!print_test_name_result(number_of_tuples == 27012,
                              "Filtered Re-partitioned Join")) {
...
static int test_complete_repartition_scan_join() {
  ...
  sb_cj_join_pair_list.push_back(LogicalEqualJoin::JoinPair(
      table_2->getAttribute("order_no"), table_1->getAttribute("order_no")));
  sb_cj_join_pair_list.push_back(
      LogicalEqualJoin::JoinPair(table_2->getAttribute("entry_date"),
                                 table_1->getAttribute("trade_date")));
  sb_cj_join_pair_list.push_back(LogicalEqualJoin::JoinPair(
      table_2->getAttribute("entry_dir"), table_1->getAttribute("trade_dir")));
  LogicalOperator* sb_cj_join = new LogicalEqualJoin(
      sb_cj_join_pair_list, sb_join_key_scan, cj_join_key_scan);
  ...
  if (!print_test_name_result(number_of_tuples == 4059609,
                              "Re-partitioned Join")) {
...
static int test_no_repartition_scan_join() {
  ...
  sb_cj_join_pair_list.push_back(LogicalEqualJoin::JoinPair(
      table_1->getAttribute("row_id"), table_2->getAttribute("row_id")));
  LogicalOperator* sb_cj_join = new LogicalEqualJoin(
      sb_cj_join_pair_list, cj_join_key_scan, sb_join_key_scan);
  ...
  if (!print_test_name_result(number_of_tuples == 944925,
                              "no_repartition Join")) {
...
static int test_expanderFramework_single_node(int repeated_times = 20) {
  ...
  //	LogicalOperator* root=convert_sql_to_logical_operator_tree("selects *
  // from cj;");
  //	if(root!=0){
  //		printf("OK!\n");
  //	}
...
static void startup_multiple_node_environment() {
  ...
  if (master != 0) {
    ...
    table_1->createHashPartitionedProjection(cj_proj0_index, "sec_code",
                                             4);  // G0
    ...
    table_2->createHashPartitionedProjection(sb_proj0_index, "sec_code",
                                             4);  // G0
...
static int test_multiple_scan_filter_high_selectivity() {
  ...
  const unsigned long int number_of_tuples =
      executable_query_plan->GetNumberOfTuples();
  if (!print_test_name_result(number_of_tuples == 14623495,
                              "High selectivity filter")) {
...
static int test_multiple_scan_filter_low_selectivity() {
  ...
  const unsigned long int number_of_tuples =
      executable_query_plan->GetNumberOfTuples();
  if (!print_test_name_result(number_of_tuples == 104010,
                              "Low selectivity filter")) {
...
static int test_multiple_scan_filter_Aggregation() {
  ...
  const unsigned long int number_of_tuples =
      executable_query_plan->GetNumberOfTuples();
  if (!print_test_name_result(number_of_tuples == 3480,
                              "Filtered Aggregation")) {
...
static int test_multiple_no_repartition_filtered_join() {
  ...
  if (!print_test_name_result(number_of_tuples == 278,
                              "Filtered no_repartition Join")) {
...
static int test_multiple_complete_repartition_filtered_join() {
  ...
  if (!print_test_name_result(number_of_tuples == 105480,
                              "Filtered Re-partitioned Join")) {
...
static int test_multiple_complete_repartition_scan_join() {
  ...
  if (!print_test_name_result(number_of_tuples == 16257984,
                              "Re-partitioned Join")) {
...
static int test_multiple_expanderFramework_single_node(
    int repeated_times = 20) {
