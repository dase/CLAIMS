...
static double lineitem_scan_self_join() {
  ...
  s_ps_join_condition.push_back(
      LogicalEqualJoin::JoinPair(table->getAttribute("L_ORDERKEY"),
                                 table_right->getAttribute("L_ORDERKEY")));
  ...
  LogicalOperator* s_ps_join =
      new LogicalEqualJoin(s_ps_join_condition, scan, scan_right);

  LogicalOperator* root = new LogicalQueryPlanRoot(
      0, s_ps_join, LogicalQueryPlanRoot::kResultCollector);
...
static double sb_scan_self_join() {
  TableDescriptor* table =
      Environment::getInstance()->getCatalog()->getTable("cj");
  TableDescriptor* table_right =
  ...
  s_ps_join_condition.push_back(LogicalEqualJoin::JoinPair(
      table->getAttribute("order_no"), table_right->getAttribute("order_no")));
  s_ps_join_condition.push_back(
      LogicalEqualJoin::JoinPair(table->getAttribute("trade_dir"),
                                 table_right->getAttribute("entry_dir")));
  s_ps_join_condition.push_back(
      LogicalEqualJoin::JoinPair(table->getAttribute("trade_date"),
                                 table_right->getAttribute("entry_date")));
  LogicalOperator* s_ps_join =
      new LogicalEqualJoin(s_ps_join_condition, scan, scan_right);
...
static double lineitem_scan_aggregation() {
  ...
  LogicalOperator* aggregation = new LogicalAggregation(
      group_by_attributes, aggregation_attributes, aggregation_function, scan);

  LogicalOperator* root = new LogicalQueryPlanRoot(
      0, aggregation, LogicalQueryPlanRoot::kResultCollector);
...
static double lineitem_scan_filter() {
  ...
  LogicalOperator* root = new LogicalQueryPlanRoot(
      0, filter, LogicalQueryPlanRoot::kResultCollector);
...
static void scalability_test(query_function qf, const char* test_name,
                             int max_test_degree_of_parallelism = 4) {
  ...
  for (unsigned i = 1; i <= max_test_degree_of_parallelism; i++) {
    ...
    if (i == 1) {
      ...
      printf("D=%d\ts=%4.4f scale:%f\n", i, total_time / repeated_times,
             1 / (total_time / repeated_times) / standard_throughput);
    }
  }
}
...
static int in_segment_scalability_test_on_tpch(int repeated_times = 10) {
  ...
  scalability_test(lineitem_scan_self_join, "Scan-->join",
...
static int in_segment_scalability_test_on_poc(int repeated_times = 10) {
  ...
  scalability_test(sb_scan_filter, "Scan-->filter",
                   Config::max_degree_of_parallelism);
  scalability_test(sb_scan_aggregation, "Scan-->aggregation",
                   Config::max_degree_of_parallelism);
  scalability_test(sb_scan_self_join, "Scan-->join",
                   Config::max_degree_of_parallelism);
