...
enum ResponseStatus { ERROR = 0, OK, SCHEMASS, HEADER, DATA, ENDED, CHANGEDD };

struct ColumnHeader {
  std::vector<std::string> header_list;
  void add_header(std::string name) { header_list.push_back(name); }
...
struct ClientResponse {
  ...
  BlockStreamBase* getDataBlock(Schema* schema) const {
    ...
    //		BlockStreamBase* ret = BlockStreamBase::createBlock(schema,
    // length-sizeof(int));
