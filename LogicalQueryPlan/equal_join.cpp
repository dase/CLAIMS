...
bool EqualJoin::canLeverageHashPartition(
  ...
  for (unsigned i = 0; i < partition_key_list.size(); i++) {
    if (attribute == partition_key_list[i]) return true;
  }
  return false;
}
...
BlockStreamIteratorBase* EqualJoin::getIteratorTree(
   ...
   * cache line
   * size, e.g., 64Bytes. Finally, I realized that different from aggregation,
   * the hash
   * table bucket in the build phase of hash join is filled very quickly and
   * hence a
   * a relatively large bucket size could reduce the number of overflowing
   * buckets and
