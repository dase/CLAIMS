...
static int testSort() {
  ...
  if (master != 0) {
    ...
    filter_condition_1.add(table_1->getAttribute("row_id"),
                           AttributeComparator::L, std::string("100000"));

    LogicalOperator* filter_1 = new Filter(filter_condition_1, scan);

    vector<LogicalSort::OrderByAttr*> oas;
    LogicalSort::OrderByAttr* od0 =
        new LogicalSort::OrderByAttr("cj", "row_id");
    LogicalSort::OrderByAttr* od1 =
        new LogicalSort::OrderByAttr("cj", "order_no");
    LogicalSort::OrderByAttr* od2 =
        new LogicalSort::OrderByAttr("cj", "sec_code");
    oas.push_back(od2);
    oas.push_back(od1);
    oas.push_back(od0);
    LogicalOperator* sort = new LogicalSort(scan, oas);

    BlockStreamIteratorBase* sort_ =
        sort->getIteratorTree(64 * 1024 - sizeof(unsigned));
