...
static int aggregation_test() {
  ...
  if (master != 0) {
    ...
    table_1->createHashPartitionedProjection(cj_proj0_index, "order_no",
                                             1);  // G0
    //		table_1->createHashPartitionedProjection(cj_proj0_index,"row_id",1);
    ////G0
    ...
    //		table_2->createHashPartitionedProjection(sb_proj0_index,"order_no",8);
    ////G2
    ...
    LogicalOperator* cj_payload_scan =
        new LogicalScan(table_1->getProjectoin(1));

    LogicalOperator* sb_payload_scan =
        new LogicalScan(table_2->getProjectoin(1));
    ...
    LogicalOperator* filter_1 =
        new Filter(filter_condition_1, cj_join_key_scan);
    ...
    filter_condition_2.add(table_2->getAttribute(3), AttributeComparator::GEQ,
                           std::string("600036"));
    LogicalOperator* filter_2 =
        new Filter(filter_condition_2, sb_join_key_scan);

    Filter::Condition filter_condition_cj_payload;
    long tmp1 = 0;
    filter_condition_cj_payload.add(table_1->getAttribute(0),
                                    AttributeComparator::EQ, &tmp1);
    LogicalOperator* filter_cj_payload =
        new Filter(filter_condition_cj_payload, cj_payload_scan);

    Filter::Condition filter_condition_sb_payload;
    long tmp2 = 0;
    filter_condition_sb_payload.add(table_2->getAttribute(0),
                                    AttributeComparator::EQ, &tmp2);
    LogicalOperator* filter_sb_payload =
        new Filter(filter_condition_sb_payload, sb_payload_scan);
    ...
    sb_cj_join_pair_list.push_back(EqualJoin::JoinPair(
        table_1->getAttribute("order_no"), table_2->getAttribute("order_no")));
    sb_cj_join_pair_list.push_back(
        EqualJoin::JoinPair(table_1->getAttribute("trade_date"),
                            table_2->getAttribute("entry_date")));
    sb_cj_join_pair_list.push_back(
        EqualJoin::JoinPair(table_1->getAttribute("trade_dir"),
                            table_2->getAttribute("entry_dir")));
    ...
    LogicalOperator* sb_cj_join =
        new EqualJoin(sb_cj_join_pair_list, filter_1, filter_2);
    ...
    cj_payload_join_pari_list.push_back(EqualJoin::JoinPair(
        table_1->getAttribute("row_id"), table_1->getAttribute("row_id")));
    LogicalOperator* cj_payload_join =
        new EqualJoin(cj_payload_join_pari_list, sb_cj_join, cj_payload_scan);

    std::vector<EqualJoin::JoinPair> sb_payload_join_pari_list;
    sb_payload_join_pari_list.push_back(EqualJoin::JoinPair(
        table_2->getAttribute("row_id"), table_2->getAttribute("row_id")));
    LogicalOperator* sb_payload_join = new EqualJoin(
        sb_payload_join_pari_list, cj_payload_join, sb_payload_scan);
    ...
    //Aggregation(group_by_attributes,aggregation_attributes,aggregation_function,cj_join_key_scan);
    //		LogicalOperator* aggregation=new
    //Aggregation(group_by_attributes,aggregation_attributes,aggregation_function,cj_join_key_scan);
    ...
    //		std::vector<BlockStreamAggregationIterator::State::aggregation>
    //aggregation_function_1;
    //		aggregation_function_1.push_back(BlockStreamAggregationIterator::State::count);
    //
    //		LogicalOperator* aggregation_1=new
    //Aggregation(std::vector<Attribute>(),aggregation_attributes_1,aggregation_function_1,cj_join_key_scan);

    const NodeID collector_node_id = 0;
    LogicalOperator* root = new LogicalQueryPlanRoot(
        collector_node_id, aggregation, LogicalQueryPlanRoot::PRINT,
        LimitConstraint(5, 10));
