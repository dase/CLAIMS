...
//		pb->BindingEntireProjection(catalog->getTable(1)->getProjectoin(3)->getPartitioner(),MEMORY);
		...
		LogicalOperator* sb_join_key_scan=new LogicalScan(table_2->getProjectoin(4));
...
//			BlockStreamIteratorBase* executable_query_plan=root->getIteratorTree(1024*64-sizeof(unsigned));
		...
		while(c==1){
			timer_start=curtick();
			IteratorExecutorMaster::getInstance()->ExecuteBlockStreamIteratorsOnSite(executable_query_plan,1);//						executable_query_plan->open();//			while(executable_query_plan->next(0));
