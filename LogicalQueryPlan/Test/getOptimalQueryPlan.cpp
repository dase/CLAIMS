 ...
 *      Author: wangli
 */
#ifndef __TESTGETOPTIMALQUERYPLAN__
...
static int getOptimalQueryPlan() {
  ...
  if (master != 0) {
    ...
    LogicalOperator* cj_join_key_scan =
        new LogicalScan(table_1->getProjectoin(4));
    LogicalOperator* sb_join_key_scan =
        new LogicalScan(table_2->getProjectoin(4));
    ...
    if (root->GetOptimalPhysicalPlan(req, physical_plan,
