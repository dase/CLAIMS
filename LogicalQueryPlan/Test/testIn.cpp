...
static int in_test() {
  ...
  if (master != 0) {
    ...
    filter_condition_1.add(table_1->getAttribute(0), AttributeComparator::L,
                           std::string(
                               "5"
                               ""));
    ...
    LogicalOperator* aggregation = new Aggregation(
    ...
    join_pair_list.push_back(
        EqualJoin::JoinPair(table_1->getAttribute("row_id"),
                            aggregation->getDataflow().getAttribute("row_id")));
    LogicalOperator* sb_cj_join =
        new EqualJoin(join_pair_list, cj_join_key_scan, aggregation);

    const NodeID collector_node_id = 0;
    LogicalOperator* root = new LogicalQueryPlanRoot(
        collector_node_id, sb_cj_join, LogicalQueryPlanRoot::PRINT);
    unsigned long long int timer_start = curtick();
    //		root->print();

    //		const NodeID collector_node_id=0;
    //		LogicalOperator* root1=new
    //LogicalQueryPlanRoot(0,sb_cj_join,LogicalQueryPlanRoot::PRINT);
    ...
    while (c == 1) {
      timer_start = curtick();
      //			IteratorExecutorMaster::getInstance()->ExecuteBlockStreamIteratorsOnSite(root1->getIteratorTree(1024*64-sizeof(unsigned)),"127.0.0.1");//
