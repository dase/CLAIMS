...
static int query_optimization_based_on_statistics_join() {
  ...
  if (master != 0) {
    ...
    while (cout) {
      ...
      //			if(node==0){
      //				printf("Error occurs while parsing the
      //sql.\n");
      //				break;
      //			}
      //			output(node,0);
      //			puts("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\n");
      //			LogicalOperator*
      //plan=parsetree2logicalplan(node);
      ...
      sb_cj_join_pair_list.push_back(
          EqualJoin::JoinPair(table_1->getAttribute("order_no"),
                              table_2->getAttribute("order_no")));
      sb_cj_join_pair_list.push_back(
      ...
      LogicalOperator* root = new LogicalQueryPlanRoot(
          0, sb_cj_join, LogicalQueryPlanRoot::PERFORMANCE);
...
static int query_optimization_based_on_statistics_aggregation() {
  ...
  if (master != 0) {
    ...
    Attribute att;
    //		Analyzer::analyse(table_id,Analyzer::a_l_attribute);
    Analyzer::analyse_on_project(
        catalog->getTable("cj")->getProjectoin(0)->getProjectionID());
    ...
    while (cout) {
      ...
      LogicalOperator* cj_join_key_scan =
          new LogicalScan(table_1->getProjectoin(6));
      Filter::Condition filter_condition_1;
      //			const int order_type=1;
      filter_condition_1.add(table_1->getAttribute(5), AttributeComparator::EQ,
                             std::string("1"));
...
static int query_optimization_based_on_statistics() {
  ...
  if (master != 0) {
    ...
    /*
     * 输入sql语句为：
    std::string create_projection="CREATE PROJECTION ON cj (row_id,trade_date)
    [number=1] partitioned on row_id";
    number 是
    partition_key_index，是createHashPartitionedProjection的第三个参数。
...
static int query_optimization_outputdata() {
  ...
  if (master != 0) {
    ...
    /** the following two lines are moved to the initialization code in
     * Environment.*/
    // Catalog* catalog=Environment::getInstance()->getCatalog();
    ...
    //		TableDescriptor* table_1=new
    //TableDescriptor("cj",Environment::getInstance()->getCatalog()->allocate_unique_table_id());
    //		/**
    //		 * @li: I change the following code such that the scheme matches those
    //in cj's first projection
    //		 */
    //		table_1->addAttribute("row_id",data_type(t_u_long));
    ////0
    //		table_1->addAttribute("trade_date",data_type(t_int));
    ...
    //		table_1->createHashPartitionedProjection(cj_proj0,"row_id",1);
    ////G0
    //		catalog->add_table(table_1);
    //
    //		for(unsigned
