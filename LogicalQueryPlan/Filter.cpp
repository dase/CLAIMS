...
Filter::Filter(LogicalOperator *child,
               std::vector<std::vector<ExpressionItem> > &exprArray)
    : child_(child),
      exprArray_(exprArray) {
...
Filter::Filter(std::vector<AttributeComparator> ComparatorList,
    ...
    : comparator_list_(ComparatorList),
      child_(child) {
  assert(!comparator_list_.empty());
  setOperatortype(l_filter);
}

Filter::Filter(const Condition& condition, LogicalOperator* child)
...
Dataflow Filter::GetDataflow() {
  ...
  if (comparator_list_.size() == 0) generateComparatorList(dataflow);
...
bool Filter::GetOptimalPhysicalPlan(
  ...
  if (child_->GetOptimalPhysicalPlan(Requirement(), physical_plan)) {
    ...
    if (transfer == NONE) {
      ...
      state.colindex_ = colindex_;
      state.comparator_list_ = comparator_list_;
      state.v_ei_ = exprArray_;   // not used in current version
      Dataflow dataflow = GetDataflow();
      state.schema_ = GetSchema(dataflow.attribute_list_);
      BlockStreamIteratorBase* filter = new ExpandableBlockStreamFilter(state);
...
void Filter::Condition::add(const Attribute& attr,
                            const AttributeComparator::comparison& com,
                            const void* const & const_value) {
...
void Filter::Condition::print(int level) const {
  ...
  for (unsigned i = 0; i < attribute_list_.size(); i++) {
    ...
    printf(
        "%s\n",
        attribute_list_[i].attrType->operate->toString(const_value_list_[i])
            .c_str());
  }
...
void Filter::generateComparatorList(const Dataflow& dataflow) {
  for (unsigned i = 0; i < condition_.getCompaisonNumber(); i++) {
    ...
    for (; attribute_index < dataflow.attribute_list_.size();
        attribute_index++) {
      if (condition_.attribute_list_[i]
          == dataflow.attribute_list_[attribute_index]) {
        break;
      }
    }
    if (attribute_index == dataflow.attribute_list_.size()) {
      printf(
          "the filter condition %s does match any input attribute! Rechech the filter condition or the filter transformation module.\n",
          condition_.attribute_list_[i].attrName.c_str());
      assert(false);
    }
    AttributeComparator filter(
        *dataflow.attribute_list_[attribute_index].attrType,
        condition_.comparison_list_[i], attribute_index,
