    ...
    if (transfer == NONE) {
      ...
      if (requirement.hasRequiredLocations()) {
        ...
        if (requirement.hasRequiredPartitionFunction()) {
          ...
          // TODO(anyone): decide the degree of parallelism
...
/*
 * TODO(Anybody)
 * couldHashPruned is remained to be rewrite， for condition_ is removed in the current
 * version, and there not exist a new method.
 */
...
/*
 * TODO(Anybody): predictSelectivity  is remained to be rewrite， for condition_ is removed in the current
 * version, and there not exist a new method.
 */
float Filter::PredictSelectivity() const {
  ...
  // because condition will be abandoned,
  // the following code is actually not used
//  for (unsigned i = 0; i < condition_.getCompaisonNumber(); i++) {
//    float selectivity = 1;
...
//           * to predict the selectivity.
//           * In such case, we assume each distinct value has the same
//           * frequency, i.e., cardinality/distinct_cardinality.
//           */
