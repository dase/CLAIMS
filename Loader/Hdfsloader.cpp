...
bool HdfsLoader::insertRecords(){
	...
	{
	cout << "The record is null!\n";
	assert(false);
	exit(-1);
	}
	void *tuple_buffer = malloc(table_descriptor_->getSchema()->getTupleMaxSize());
	...
	table_descriptor_->getSchema()->toValue(s_record, tuple_buffer, col_separator);
	row_id++;
...
/*for testing end*/
	...
	for (int i = 0; i < table_descriptor_->getNumberOfProjection(); i++)
	{
		...
		void* target = malloc(table_descriptor_->getProjectoin(i)->getSchema()->getTupleMaxSize());
		...
		void* partition_key_addr=table_descriptor_->getProjectoin(i)->getSchema()->getColumnAddess(partition_key_local_index,target);
		part=table_descriptor_->getProjectoin(i)->getSchema()->getcolumn(partition_key_local_index).operate->getPartitionValue(partition_key_addr, partition_functin_list_[i]);

		void* block_tuple_addr = pj_buffer[i][part]->allocateTuple(table_descriptor_->getProjectoin(i)->getSchema()->getTupleMaxSize());
		if(block_tuple_addr == 0)
		{
			...
			block_tuple_addr = pj_buffer[i][part]->allocateTuple(table_descriptor_->getProjectoin(i)->getSchema()->getTupleMaxSize());
		}
		table_descriptor_->getProjectoin(i)->getSchema()->copyTuple(target,block_tuple_addr);
