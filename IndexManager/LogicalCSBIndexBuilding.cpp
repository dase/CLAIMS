...
LogicalCSBIndexBuilding::LogicalCSBIndexBuilding(ProjectionID projection_id,
  ...
  scan_projection_ =
      Catalog::getInstance()->getTable(index_attr_.table_id_)->getProjectoin(
          projection_id_.projection_off);
...
PlanContext LogicalCSBIndexBuilding::GetPlanContext() {
  if (!scan_projection_->AllPartitionBound()) {
    Catalog::getInstance()->getBindingModele()->BindingEntireProjection(
        scan_projection_->getPartitioner(), DESIRIABLE_STORAGE_LEVEL);
  }
...
PhysicalOperatorBase* LogicalCSBIndexBuilding::GetPhysicalPlan(
...
bool LogicalCSBIndexBuilding::GetOptimalPhysicalPlan(
...
void LogicalCSBIndexBuilding::Print(int level) const {
  printf("%*.CSBIndexBuilding: %s\n", level * 8, " ",
         Catalog::getInstance()
             ->getTable(scan_projection_->getProjectionID().table_id)
             ->getTableName()
             .c_str());
}
