 ...
 *
 * /CLAIMS/logical_operator/logical_left_join.cpp
 *
...
void LogicalOuterJoin::DecideJoinPolicy(const PlanContext& left_dataflow,
...
bool LogicalOuterJoin::IsHashOnLeftKey(const Partitioner& part,
...
bool LogicalOuterJoin::CanOmitHashRepartition(
...
bool LogicalOuterJoin::IsInOneJoinPair(
    const Attribute& left_partition_key,
...
LogicalOuterJoin::JoinPolicy LogicalOuterJoin::DecideLeftOrRightRepartition(
...
PhysicalOperatorBase* LogicalOuterJoin::GetPhysicalPlan(
    const unsigned& block_size) {
  if (NULL == plan_context_) {
    GetPlanContext();
  }
  PhysicalOuterHashJoin* join_iterator;
...
bool LogicalOuterJoin::GetOptimalPhysicalPlan(
    Requirement requirement, PhysicalPlanDescriptor& physical_plan_descriptor,
    const unsigned& block_size) {}

std::vector<unsigned> LogicalOuterJoin::GetLeftJoinKeyIds() const {
...
std::vector<unsigned> LogicalOuterJoin::GetRightJoinKeyIds() const {
  std::vector<unsigned> ret;
...
std::vector<unsigned> LogicalOuterJoin::GetLeftPayloadIds() const {
  std::vector<unsigned> ret;
  const PlanContext dataflow = left_child_->GetPlanContext();
...
std::vector<unsigned> LogicalOuterJoin::GetRightPayloadIds() const {
  std::vector<unsigned> ret;
  const PlanContext dataflow = right_child_->GetPlanContext();
...
int LogicalOuterJoin::GetIdInLeftJoinKeys(const Attribute& attribute) const {
  for (unsigned i = 0; i < joinkey_pair_list_.size(); i++) {
...
int LogicalOuterJoin::GetIdInLeftJoinKeys(
...
int LogicalOuterJoin::GetIdInRightJoinKeys(const Attribute& attribute) const {
  for (unsigned i = 0; i < joinkey_pair_list_.size(); i++) {
...
int LogicalOuterJoin::GetIdInRightJoinKeys(
    const Attribute& attribute,
    const std::vector<Attribute> shadow_attribute_list) const {
  for (unsigned i = 0; i < joinkey_pair_list_.size(); i++) {
    if (joinkey_pair_list_[i].right_join_attr_ == attribute) {
...
PlanPartitioner LogicalOuterJoin::DecideOutputDataflowProperty(
...
void LogicalOuterJoin::Print(int level) const {
...
double LogicalOuterJoin::PredictEqualJoinSelectivity(
    const PlanContext& left_dataflow, const PlanContext& right_dataflow) const {
  /**
