...
join_num_size_case=(
			(1048576, 1024, ((1, 1, "hash_join_s_sb"), (1, 1, "hash_join_s_bs"), )),
...
def autotest():
    for i in join_num_size_case:
        #command1=config_disk_mode%(user, hostname, i[0])+config_data%(user, hostname, i[1])+deploy%(user, hostname)
        command1=config_join_num%(user, hostname, i[0])+config_join_size%(user, hostname, i[1])+deploy%(user, hostname)
        print command1
        os.system(command1)
        time.sleep(5)
        for j in i[2]:
