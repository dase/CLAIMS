...
static int Project_test() {
  ...
  table_1->createHashPartitionedProjection(cj_proj0_index,
                                           partition_key_index_1, 1);  // G0
  ...
  //	cout<<"middle binding in the sort
  //test!!!"<<"ooo"<<partition_key_index_1<<endl;
  ...
  ExpandableBlockStreamProjectionScan::State scan_state(
      ...
      new SchemaFix(column_list), 64 * 1024 - sizeof(unsigned));
  ...
  BlockStreamProjectIterator::State project_state(
      new SchemaFix(column_list), new SchemaFix(column_list_), scan,
      64 * 1024 - sizeof(unsigned), *map, express_item_list);
  BlockStreamIteratorBase* project =
      new BlockStreamProjectIterator(project_state);
