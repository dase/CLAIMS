...
static int expanded_iterators_test() {
  ...
  if (master != 0) {
    ...
    table_2->createHashPartitionedProjection(sb_proj1_index, "order_no",
                                             1);  // G1
    ...
    //Aggregation(group_by_attributes,aggregation_attributes,aggregation_function,filter_1);
    //		LogicalOperator* aggregation=new
    ...
    LogicalOperator* aggregation =
        new Aggregation(group_by_attributes, aggregation_attributes,
                        aggregation_function, sb_cj_join);

    //

    //		const NodeID collector_node_id=0;
    //		LogicalOperator* root=new
    //LogicalQueryPlanRoot(collector_node_id,cj_join_key_scan,LogicalQueryPlanRoot::PERFORMANCE);
    ...
    executable_query_plan =
        target->getIteratorTree((1024 * 64 - sizeof(unsigned)));

    BlockStreamExpander::State expander_state(target->getDataflow().getSchema(),
                                              executable_query_plan, 500,
                                              1024 * 64 - sizeof(unsigned));
    BlockStreamIteratorBase* expander = new BlockStreamExpander(expander_state);

    BlockStreamPerformanceMonitorTop::State perf_state(
        target->getDataflow().getSchema(), expander,
        1024 * 64 - sizeof(unsigned));
    BlockStreamIteratorBase* top =
        new BlockStreamPerformanceMonitorTop(perf_state);

    top->print();

    BlockStreamBase* block =
        BlockStreamBase::createBlockWithDesirableSerilaizedSize(
            target->getDataflow().getSchema(), 1024 * 64);
    int c = 1;
    while (c == 1) {
