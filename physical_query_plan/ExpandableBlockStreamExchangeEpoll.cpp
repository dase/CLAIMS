...
bool ExpandableBlockStreamExchangeEpoll::Next(BlockStreamBase* block) {
  while (true) {
    ...
    if (nexhausted_lowers == nlowers &&
      ...
      logging_->log("the value of sem_new_block_or_eof_ is :%d",
                    sem_new_block_or_eof_.get_value());
      logging_->log("now all lower are exhausted~~~~~~~~~~~");
      return false;
    }
  }
}
        ...
        while (true) {
          ...
          if (!eof) {
            ...
            if (Pthis->nexhausted_lowers == Pthis->nlowers) {
              ...
              oss << "Var:" << get_stddev(finish_times) << "\n";
              //              cout << oss.str();

              Pthis->logging_->log("%s", oss.str().c_str());

              //              LOG(INFO) << oss.str();
              //              Pthis->logging_->log("\t Var:%5.4f\n",
            ...
            //            LOG(INFO) << "test" << endl;
            //            LOG(INFO) << "[" << Pthis->state.exchange_id_
            //                      << "] This notification (all the blocks in
            //                      the socket "
            //                         "buffer are consumed) is send to the
            //                         lower["
            //                      <<
            //                      Pthis->lower_ip_array[socket_fd_index].c_str()
            //                      << "] exchange=(" <<
            //                      Pthis->state.exchange_id_ << ","
            //                      << Pthis->partition_offset << ").\n";
          }
        }
        if (done) {
          printf(
