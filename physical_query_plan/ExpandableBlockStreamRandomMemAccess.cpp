...
bool ExpandableBlockStreamRandomMemAccess::Open(
  ...
  if (atomicPopRemainingBlock(rb)) {
    while ((tuple_from_child = rb.iterator->currentTuple()) > 0) {
      ...
      if ((tuple_in_block = block->allocateTuple(bytes)) > 0) {
        ...
        state_.f_schema_->copyTuple(
            (void*)((char*)base_ + (*(int*)tuple_from_child) * bytes),
            tuple_in_block);
...
bool ExpandableBlockStreamRandomMemAccess::atomicPopRemainingBlock(
    remaining_block& rb) {
  lock_.acquire();
...
void ExpandableBlockStreamRandomMemAccess::atomicPushRemainingBlock(
    remaining_block rb) {
  lock_.acquire();
...
BlockStreamBase*
ExpandableBlockStreamRandomMemAccess::AtomicPopFreeBlockStream() {
...
void ExpandableBlockStreamRandomMemAccess::AtomicPushFreeBlockStream(
