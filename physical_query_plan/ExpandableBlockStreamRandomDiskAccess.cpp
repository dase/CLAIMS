...
#include "../physical_query_plan/ExpandableBlockStreamRandomDiskAccess.h"
...
ExpandableBlockStreamRandomDiskAccess::State::State(
    std::string file_name, BlockStreamIteratorBase* child, Schema* f_schema,
    Schema* c_schema, unsigned block_size)
    : filename_(file_name),
      child_(child),
      f_schema_(f_schema),
      c_schema_(c_schema),
...
bool ExpandableBlockStreamRandomDiskAccess::Open(
  ...
  AtomicPushFreeBlockStream(
      BlockStreamBase::createBlock(state_.c_schema_, state_.block_size_));
...
bool ExpandableBlockStreamRandomDiskAccess::Next(BlockStreamBase* block) {
  ...
  if (atomicPopRemainingBlock(rb)) {
    while ((tuple_from_child = rb.iterator->currentTuple()) > 0) {
      const unsigned bytes =
          state_.f_schema_->getTupleActualSize(tuple_in_block);
      if ((tuple_in_block = block->allocateTuple(bytes)) > 0) {
        ...
        fread(tuple_in_block, 1,
              state_.f_schema_->getTupleActualSize(tuple_in_block), fd_);
        //				state_.f_schema_->copyTuple((void*)(base_+(*(int*)tuple_from_child)*bytes),
        //tuple_in_block);
  ...
  /* When the program arrivals here, it mains that there is no remaining blocks
   * or the remaining block
  ...
  while (state_.child_->Next(block_for_asking)) {
    ...
    while ((tuple_from_child = traverse_iterator->currentTuple()) > 0) {
      const unsigned bytes =
          state_.f_schema_->getTupleActualSize(tuple_from_child);
      if ((tuple_in_block = block->allocateTuple(bytes)) > 0) {
        /* the block has enough space to hold this tuple */
        fseek(fd_, (*(int*)tuple_from_child) * bytes, SEEK_SET);
        fread(tuple_in_block, 1,
        ...
        atomicPushRemainingBlock(
            remaining_block(block_for_asking, traverse_iterator));
        //				cout << "return 2 true\n";
        return true;
      }
    }
...
bool ExpandableBlockStreamRandomDiskAccess::atomicPopRemainingBlock(
    remaining_block& rb) {
  lock_.acquire();
...
void ExpandableBlockStreamRandomDiskAccess::atomicPushRemainingBlock(
    remaining_block rb) {
  lock_.acquire();
...
BlockStreamBase*
ExpandableBlockStreamRandomDiskAccess::AtomicPopFreeBlockStream() {
...
void ExpandableBlockStreamRandomDiskAccess::AtomicPushFreeBlockStream(
