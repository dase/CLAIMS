...
void PhysicalOperator::UnregisterExpandedThreadToAllBarriers(unsigned barrier_index){
	number_of_registered_expanded_threads_--;
	lock_number_of_registered_expanded_threads_.release();
...
void PhysicalOperator::DestoryAllContext(){
	for(boost::unordered_map<pthread_t,ThreadContext*>::const_iterator it=context_list_.begin();it!=context_list_.cend();it++){
		delete it->second;
	}
...
void PhysicalOperator::SetReturnStatus(bool ret) {
	ret=open_ret_&&ret;
}

ThreadContext* PhysicalOperator::CreateOrReuseContext(
...
bool PhysicalOperator::GetReturnStatus() const {
	return open_ret_;
}

ThreadContext* PhysicalOperator::GetFreeContext(
		context_reuse_mode crm) {
