...
ExpandableBlockStreamHdfsScan::State::State(string partition_file_name,
...
bool ExpandableBlockStreamHdfsScan::Open(const PartitionOffset &) {
  if (sema_open_.try_wait()) {
    ...
    //		vector<HdfsBlock>
    // hdfsblock_array=ms->getFromMem(state_.partition_file_name_);
    //		cout<<hdfsblock_array.size()<<endl;
    BlockManager::BlockManagerWorkerActor *bmk = 0;
    BlockManager *bm = BlockManager::getInstance();
    /*		vector<ChunkInfo>
     * vci=bm->loadFromHdfs(state_.partition_file_name_.c_str());*/
    // 在此将所有的指针都存进去，非空即存
    int offset = 0;
    void *chunkptr;
    bool bl = true;
    string filename =
        bm->askForMatch(state_.partition_file_name_.c_str(), *(bm->getId()));
...
bool ExpandableBlockStreamHdfsScan::Next(BlockStreamBase *block) {
  //	void
  //*toTuple=memalign(cacheline_size,state_.partition_schema_->getTupleMaxSize());
  //	void *column_in_toTuple;
  allocated_block allo_block_temp;
  if (cb_.file_index < file_set_.size()) {
    // chunk还有剩余位置存放其他block。 -- Han
    lock_.acquire();
    if (cb_.cursor >= cb_.base + cb_.length) {
      //判断当前光标位置是否超出指定长度,若超出则表示块已经满。 --Han
      cb_.file_index++;
      cb_.cursor = cb_.base = file_set_[cb_.file_index];
      //将光标当前位置置为下一个block所在位置。  --Han
      cb_.length = 64 * 1024 * 1024;  // CHUNK_SIZE
    }
    lock_.release();
    if (cb_.file_index < file_set_.size()) {
  ...
  //			for(unsigned
  // i=0;i<state_.partition_schema_->getncolumns();i++){
...
bool ExpandableBlockStreamHdfsScan::atomicIncreaseCursor(
    unsigned bytes, allocated_block &allo_block) {
  bool ret;
  lock_.acquire();
  //加入新的bytes没有超出当前block，则加入缓冲区。--H
  ...
  /* there some data remaining, but the data is smaller than the expected
     bytes.*/
  else if (cb_.cursor < cb_.base + cb_.length) {
    //此时表示block有内碎片，需要将当前的block加入到缓冲区去。-H
    allo_block.start = cb_.cursor;
    allo_block.length = cb_.base + cb_.length - cb_.cursor;
    cb_.cursor += allo_block.length;
    ret = true;
  } else {
    //真的真的装不下啦，而且是刚刚好的完成了一个block -H
    ret = false;
  }
  lock_.release();
  return ret;
