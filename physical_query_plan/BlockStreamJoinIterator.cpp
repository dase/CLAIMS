...
bool BlockStreamJoinIterator::Open(const PartitionOffset& partition_offset) {
  ...
  if (TryEntryIntoSerializedSection(0)) {
    ...
    hash =
        PartitionFunctionFactory::createBoostHashFunction(state_.ht_nbuckets);
    ...
    QNode* expr = createEqualJoinExpression(
        state_.ht_schema, state_.input_schema_right, state_.joinIndex_left,
  ...
  const Operate* op = input_schema->getcolumn(state_.joinIndex_left[0])
  ...
  while (state_.child_left->Next(jtc->l_block_for_asking_)) {
    ...
    while (cur = jtc->l_block_stream_iterator_->nextTuple()) {
      ...
      //			for(unsigned
      //i=0;i<state_.payload_left.size();i++){
...
bool BlockStreamJoinIterator::Next(BlockStreamBase* block) {
  ...
  void* joinedTuple =
      memalign(cacheline_size, state_.output_schema->getTupleMaxSize());
  bool key_exit;
  ...
  while (true) {
    while ((tuple_from_right_child =
      ...
      unsigned bn =
          state_.input_schema_right->getcolumn(state_.joinIndex_right[0])
              .operate->getPartitionValue(
                  state_.input_schema_right->getColumnAddess(
                      state_.joinIndex_right[0], tuple_from_right_child),
                  state_.ht_nbuckets);

      while ((tuple_in_hashtable = jtc->hashtable_iterator_.readCurrent()) >
             0) {
...
inline void BlockStreamJoinIterator::isMatch(
...
ThreadContext* BlockStreamJoinIterator::CreateContext() {
