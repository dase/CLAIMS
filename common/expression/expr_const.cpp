...
ExprConst::ExprConst(ExprNodeType expr_node_type, data_type actual_type,
        ...
        : ExprNode(expr_node_type, actual_type, alias),
          const_value_(string(const_value)) {
}
ExprConst::ExprConst(ExprConst* expr)
        : ExprNode(expr),
          const_value_(expr->const_value_) {
}
void* ExprConst::ExprEvaluate(void* tuple, Schema* schema) {
    return value_;
}
 ...
 *  so later, just to return the value_ is ok, because this can avoid type_casted
 *  everytime.
 */
