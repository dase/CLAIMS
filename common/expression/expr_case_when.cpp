...
ExprCaseWhen::ExprCaseWhen(ExprCaseWhen* expr)
        : ExprNode(expr),
          mywhen_(expr->mywhen_),
          mythen_(expr->mythen_) {
    for (int i = 0; i < mywhen_.size(); i++) {
        mywhen_[i] = mywhen_[i]->ExprCoyp();
    }
    for (int i = 0; i < mythen_.size(); i++) {
        mythen_[i] = mythen_[i]->ExprCoyp();
    }
}
void* ExprCaseWhen::ExprEvaluate(void* tuple, Schema* schema) {
    ...
    void* result;
    for (int i = 0; i < mywhen_.size(); i++) {
        if (*static_cast<bool*>(mywhen_[i]->ExprEvaluate(tuple, schema))
                == true) {
            then = mythen_[i];
            break;
        }
...
void ExprCaseWhen::InitExprAtLogicalPlan(
    ...
    for (int i = 0; i < mythen_.size(); i++) {
        mythen_[i]->InitExprAtLogicalPlan(mythen_[i]->actual_type_,
                                          column_index, schema);
...
ExprNode* ExprCaseWhen::ExprCoyp() {
    return new ExprCaseWhen(this);
}
