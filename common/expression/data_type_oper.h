...
#ifndef DATA_TYPE_OPER_H_
#define DATA_TYPE_OPER_H_
#include"expr_node.h"
...
inline void int_add(FuncCallInfo fcinfo) {
    ...
    *(int *) fcinfo->results = (*(int *) fcinfo->args[0]
            + (*(int *) fcinfo->args[1]));
}
...
inline void int_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(int *) fcinfo->args[0])
            == (*(int *) fcinfo->args[1]);
}
...
inline void float_add(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(float *) fcinfo->results = ((*(float *) fcinfo->args[0])
            + (*(float *) fcinfo->args[1]));
}
...
inline void float_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = fabs(
            double((*(float *) fcinfo->args[0])
                    - (*(float *) fcinfo->args[1])))<DELTA;
}
...
inline void float_great(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(float *) fcinfo->args[0])
            > (*(float *) fcinfo->args[1]);
}
...
inline void double_add(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(double *) fcinfo->results = ((*(double *) fcinfo->args[0])
            + (*(double *) fcinfo->args[1]));
}
...
inline void double_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = fabs(
            *(double *) fcinfo->args[0] - *(double *) fcinfo->args[1]) < DELTA;
}
...
inline void double_great(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(double *) fcinfo->args[0]
            > *(double *) fcinfo->args[1]);
}
...
inline void smallInt_add(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(short int *) fcinfo->results = (*(short int *) fcinfo->args[0])
            + (*(short int *) fcinfo->args[1]);
}
...
inline void smallInt_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(short int *) fcinfo->args[0]
            == *(short int *) fcinfo->args[1]);
}
...
inline void decimal_add(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(NValue *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_add(
            *(NValue *) fcinfo->args[1]);
}
inline void decimal_minus(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(NValue *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_subtract(
            *(NValue *) fcinfo->args[1]);
}
...
inline void decimal_divide(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(NValue *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_divide(
            *(NValue *) fcinfo->args[1]);
}
inline void decimal_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_equals(
            *(NValue *) fcinfo->args[1]);
}
inline void decimal_not_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_not_equals(
            *(NValue *) fcinfo->args[1]);
}
inline void decimal_great(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_great(
            *(NValue *) fcinfo->args[1]);
}
inline void decimal_great_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_great_equals(
            *(NValue *) fcinfo->args[1]);
}
inline void decimal_less(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_less(
            *(NValue *) fcinfo->args[1]);
}
inline void decimal_less_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(NValue *) fcinfo->args[0]).op_less_equals(
            *(NValue *) fcinfo->args[1]);
}
...
inline void string_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (strcmp((char *) fcinfo->args[0],
                                        (char *) fcinfo->args[1]) == 0);
}
...
inline void string_both_trim(FuncCallInfo fcinfo) {
    ...
    for (;
            ((char *) fcinfo->args[1])[begin] == ((char *) fcinfo->args[0])[0]
                    && ((char *) fcinfo->args[1])[begin] != '\0'; begin++)
        ;
    if (end > begin)
        for (;
                ((char *) fcinfo->args[1])[end] == ((char *) fcinfo->args[0])[0]
                        && end >= 0; end--)
            ;
...
inline void string_like(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (MatchText((char *) fcinfo->args[0],
                                           strlen((char *) fcinfo->args[0]),
                                           (char *) fcinfo->args[1],
                                           strlen((char *) fcinfo->args[1]))
            == 1);
}
...
inline void date_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(date *) fcinfo->args[0]
            == *(date *) fcinfo->args[1]);
}
...
inline void date_add_day(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(date *) fcinfo->results = (*(date *) fcinfo->args[0])
            + (*(date_duration *) fcinfo->args[1]);
}
...
inline void time_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(time_duration *) fcinfo->args[0]
            == *(time_duration *) fcinfo->args[1]);
}
...
inline void datetime_equal(FuncCallInfo fcinfo) {
    assert(fcinfo->nargs == 2);
    *(bool *) fcinfo->results = (*(ptime *) fcinfo->args[0]
            == *(ptime *) fcinfo->args[1]);
}
...
inline void initialize_operator_function() {
    ...
    ExectorFunction::operator_function[t_int][oper_great_equal] =
            int_great_equal;
    ...
    ExectorFunction::operator_function[t_u_long][oper_multiply] =
            u_long_multiply;
    ...
    ExectorFunction::operator_function[t_float][oper_not_equal] =
            float_not_equal;
    ...
    ExectorFunction::operator_function[t_double][oper_multiply] =
            double_multiply;
    ...
    ExectorFunction::operator_function[t_double][oper_less_equal] =
            double_less_equal;
    ExectorFunction::operator_function[t_double][oper_negative] =
            double_negative;
    ...
    ExectorFunction::operator_function[t_smallInt][oper_multiply] =
            smallInt_multiply;
    ExectorFunction::operator_function[t_smallInt][oper_divide] =
            smallInt_divide;
    ...
    ExectorFunction::operator_function[t_boolean][oper_minus] =
            oper_not_support;
    ExectorFunction::operator_function[t_boolean][oper_multiply] =
            oper_not_support;
    ExectorFunction::operator_function[t_boolean][oper_divide] =
            oper_not_support;
    ...
    ExectorFunction::operator_function[t_string][oper_multiply] =
            oper_not_support;
    ExectorFunction::operator_function[t_string][oper_divide] =
            oper_not_support;
    ...
    ExectorFunction::operator_function[t_string][oper_not_like] =
            string_not_like;
    ...
    ExectorFunction::operator_function[t_date][oper_multiply] =
            oper_not_support;
    ...
    ExectorFunction::operator_function[t_date][oper_less_equal] =
            date_less_equal;
    ExectorFunction::operator_function[t_date][oper_negative] =
            oper_not_support;

    ExectorFunction::operator_function[t_date][oper_date_add_day] =
            date_add_day;
    ...
    ExectorFunction::operator_function[t_date][oper_date_add_year] =
            date_add_year;
    ExectorFunction::operator_function[t_date][oper_date_sub_day] =
            date_sub_day;
    ...
    ExectorFunction::operator_function[t_time][oper_less_equal] =
            time_less_equal;
