...
class OperateInt : public Operate {
 public:
  ...
  /*
   * TODO(lizhifang): if the value is more than INT_MAX, it should be set as
   * INT_MAX. May atol() will help this.
   */
...
class OperateString : public Operate {
 public:
  OperateString(unsigned size, bool _nullable = true) {
    this->nullable = _nullable;
  };
  ~OperateString(){};
  inline void assignment(const void* const& src, void* const& desc) const {
  ...
  /*
   *
   * TODO(lizhifang): first to do is check the length of string,
   * if it is longer than operate's size given by construct method,
   * only copy size bytes length by using strncpy, and return true which
  ...
  column_type(data_type type, unsigned _size = 0, bool _nullable = true)
    ...
    switch (type) {
      ...
      case t_float:
        ...
        break;
      // TODO(lizhifang): give OperateString the limit of length
      case t_string:
