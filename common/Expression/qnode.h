...
class QExpr_unary : public QNode {
 public:
  ...
  QExpr_unary(){};
  virtual ~QExpr_unary() { delete next; };
  QExpr_unary(QNode *arg, data_type a_type, oper_type op_types,
              qnodetype q_type, char *t_alias);
...
class QExpr_binary : public QNode  //
 ...
 public:
  ...
  QNode *lnext, *rnext;
  ExecFunc
      function_call;  // 通过函数可以知道具体的node类型，因此不需要进行nodetype的判断
  QExpr_binary(){};
  virtual ~QExpr_binary() {
    delete lnext;
    delete rnext;
  };
  QExpr_binary(QNode *l_arg, QNode *r_arg, data_type a_type, oper_type op_types,
               qnodetype q_type, char *t_alias);
 ...
 private:
  ...
  template <class Archive>
  void serialize(Archive &ar, const unsigned int version) {
    ar &boost::serialization::base_object<QNode>(*this) & lnext & rnext &
        op_type;
  }
};
class QExpr_ternary : public QNode {
 public:
  ...
  QExpr_ternary(QNode *arg0, QNode *arg1, QNode *arg2, data_type a_type,
                oper_type op_types, qnodetype q_type, char *t_alias);
 ...
 private:
  ...
  template <class Archive>
  void serialize(Archive &ar, const unsigned int version) {
    ar &boost::serialization::base_object<QNode>(*this) & op_type & next0 &
        next1 & next2;
  }
};
class QColcumns : public QNode  // getscalarvar
 ...
 public:
   ...
   */
  int tab_index;  // represent the logical index of the table this column
                  // belongs to.
  QColcumns(const char *tbl, const char *coln, data_type a_type,
            const char *t_alias);
...
class QExpr_in : public QNode {
 public:
  ...
  QExpr_in(vector<QNode *> &cmpnode_, vector<vector<QNode *> > &rnode_,
           char *alias_);
...
class QExpr_date_add_sub : public QNode {
 public:
  ...
  QExpr_date_add_sub(QNode *l_arg, QNode *r_arg, data_type a_type,
                     oper_type op_types, qnodetype q_type, data_type rr_type,
                     char *t_alias);
