    ...
    switch (node->type) {
        case t_expr_cal: {
            ...
            if (strcmp(calnode->sign, "ANDOP") == 0)  // now in and the two parameter
                ...
                QExpr_binary *qcalnode = new QExpr_binary(lnode, rnode,
                                                          t_boolean, oper_and,
                                                          t_qexpr_cal,
                                                          calnode->str);
                return qcalnode;
                ...
                QNode *lnode = transformqual(calnode->lnext, child);
                QNode *rnode = transformqual(calnode->rnext, child);
                data_type a_type = TypePromotion::arith_type_promotion_map[lnode
                        ->actual_type][rnode->actual_type];
                QExpr_binary *qcalnode = new QExpr_binary(lnode, rnode, a_type,
                                                          oper_add, t_qexpr_cal,
                                                          calnode->str);
                return qcalnode;
            } else if (strcmp(calnode->sign, "-") == 0) {
                QNode *lnode = transformqual(calnode->lnext, child);
                ...
                data_type a_type = TypePromotion::arith_type_promotion_map[lnode
                        ->actual_type][rnode->actual_type];
                QExpr_binary *likenode = new QExpr_binary(lnode, rnode, a_type,
                                                          oper_like,
                                                          t_qexpr_cmp,
                                                          calnode->str);
                return likenode;
                ...
                QNode *nnode = transformqual(calnode->rnext, child);
                QExpr_unary *unode = new QExpr_unary(nnode, nnode->actual_type,
                                                     oper_negative,
                                                     t_qexpr_unary,
                                                     calnode->str);
                return unode;
                ...
                for (int i = 0; i < lnode.size(); i++) {
                    data_type a_type =
                            TypePromotion::arith_type_promotion_map[lnode[i]
                                    ->actual_type][rnode[0][i]->actual_type];
                    QExpr_binary *qcalnode = new QExpr_binary(lnode[i],
                                                              rnode[0][i],
                                                              a_type,
                                                              oper_equal,
                                                              t_qexpr_cmp,
                ...
                switch (calnode->cmp) {
                    ...
                    case 3:  //"<>"
                    {
                        QExpr_binary *qcalnode = new QExpr_binary(
                                lnode, rnode, a_type, oper_not_equal,
                    ...
                    {
                        QExpr_binary *qcalnode = new QExpr_binary(
                                lnode, rnode, a_type, oper_less_equal,
                                t_qexpr_cmp, calnode->str);
                        return qcalnode;
                    }
                        break;
                    ...
                    {
                        QExpr_binary *qcalnode = new QExpr_binary(
                                lnode, rnode, a_type, oper_great_equal,
                                t_qexpr_cmp, calnode->str);
                        return qcalnode;
                    }
                        break;
        ...
        case t_expr_func: {
            ...
            if (strcmp(funcnode->funname, "CASE3") == 0)  // no END in CASE...WHEN
                ...
                return cwnode;
                    ...
                    {
            } else if (strcmp(funcnode->funname, "FSUBSTRING0") == 0)  // TODO the max length is 128,but it may be larger in
                                                                       // practice
                    {
                ...
                QExpr_ternary *substrnode = new QExpr_ternary(node0, node1,
                                                              node2, a_type,
                                                              oper_substring,
                                                              t_qexpr_ternary,
                                                              funcnode->str);
                ...
                data_type a_type = t_string;
                QExpr_binary *trimnode = new QExpr_binary(lnode, rnode, a_type,
                                                          oper_both_trim,
                                                          t_qexpr_cal,
                                                          funcnode->str);
                return trimnode;
                ...
                data_type a_type = t_string;
                QExpr_unary *uppernode = new QExpr_unary(nnext, a_type,
                                                         oper_upper,
                                                         t_qexpr_unary,
                                                         funcnode->str);
                return uppernode;
            ...
            } else if (strcmp(funcnode->funname, "FCOUNTALL") == 0)  // the agg nodes are changed to QColumns node ,only to get
                                                                     // value from tuple is ok
                    {
                QNode *aggnode = new QColcumns("", funcnode->str, t_u_long,
                                               funcnode->str);
                return aggnode;
                ...
                data_type a_type = child->getDataflow().getAttribute(
                        string(funcnode->str)).attrType->type;
                QNode *aggnode = new QColcumns("", funcnode->str, a_type,
                                               funcnode->str);
                return aggnode;
                ...
                data_type a_type = TypePromotion::arith_type_promotion_map[arg
                        ->actual_type][param1->actual_type];
                QNode *lnext = new QExpr_binary(arg, param1, a_type,
                                                oper_great_equal, t_qexpr_cmp,
                                                "arg>=parma1");
                QNode *rnext = new QExpr_binary(arg, param2, a_type,
                                                oper_less_equal, t_qexpr_cmp,
                                                "arg<=parma2");
                QExpr_binary *banode = new QExpr_binary(lnext, rnext, t_boolean,
                                                        oper_and, t_qexpr_cal,
                                                        funcnode->str);
                return banode;
            }
            /*
             ...
             */
                ...
                if (strcmp(datefunc->funname, "INTERVAL_DAY") == 0) {
                    ...
                    date_add = new QExpr_date_add_sub(lnext, rnext, t_date,
                                                      oper_date_add_day,
                ...
                if (strcmp(datefunc->funname, "INTERVAL_DAY") == 0) {
                    QNode *rnext = transformqual(datefunc->args, child);
                    date_sub = new QExpr_date_add_sub(lnext, rnext, t_date,
                                                      oper_date_sub_day,
                                                      t_qexpr_date_add_sub,
                                                      t_date_day,
                                                      funcnode->str);
                ...
                SQLParse_elog("get_a_expression_item: %s is null",
                              funcnode->funname);
            }
        ...
        case t_name_name: {
            ...
            data_type a_type = child->getDataflow().getAttribute(
                    string(col->parameter2)).attrType->type;
            if (col->parameter1 == NULL)  // for temporary variable
            {
                col->parameter1 = "";
            }

            QColcumns *qcol = new QColcumns(col->parameter1, col->parameter2,
                                            a_type, col->parameter2);
            return qcol;
        }
            break;
        case t_stringval: {
...
void InitExprAtLogicalPlan(QNode *node, data_type r_type,
    ...
    switch (node->type) {
        case t_qexpr_cal:  // binary calculation node
        {
            ...
            InitExprAtLogicalPlan(calnode->lnext, calnode->actual_type,
                                  colindex, schema);
            InitExprAtLogicalPlan(calnode->rnext, calnode->actual_type,
                                  colindex, schema);
            calnode->isnull =
                    (calnode->lnext->isnull || calnode->rnext->isnull);
            calnode->length = max(calnode->lnext->length,
                                  calnode->rnext->length);
        }
            break;
        case t_qexpr_cmp:  // binary comparison node
        {
            ...
            InitExprAtLogicalPlan(cmpnode->lnext, cmpnode->actual_type,
                                  colindex, schema);
            InitExprAtLogicalPlan(cmpnode->rnext, cmpnode->actual_type,
                                  colindex, schema);
            cmpnode->length = max(cmpnode->lnext->length,
                                  cmpnode->rnext->length);
            cmpnode->isnull =
                    (cmpnode->lnext->isnull || cmpnode->rnext->isnull);
        }
            break;
        case t_qexpr_unary: {
            QExpr_unary *unode = (QExpr_unary *) node;
            unode->return_type = r_type;
            InitExprAtLogicalPlan(unode->next, unode->actual_type, colindex,
                                  schema);
        ...
        case t_qexpr_ternary:  // now for substring,not for all
        {
            ...
            InitExprAtLogicalPlan(tnode->next0, tnode->actual_type, colindex,
                                  schema);
            InitExprAtLogicalPlan(tnode->next1, tnode->next1->actual_type,
                                  colindex, schema);  // parameter return type =actual type
            InitExprAtLogicalPlan(tnode->next2, tnode->next2->actual_type,
                                  colindex, schema);  // parameter return type =actual type
            tnode->length = max(
                    tnode->next0->length,
        ...
        case t_qexpr_case_when: {
            ...
            for (int i = 0; i < cwnode->qual.size(); i++) {
                InitExprAtLogicalPlan(cwnode->qual[i], t_boolean, colindex,
                                      schema);
            }
            for (int i = 0; i < cwnode->ans.size(); i++) {
                InitExprAtLogicalPlan(cwnode->ans[i],
                                      cwnode->ans[i]->actual_type, colindex,
                                      schema);
        ...
        case t_qexpr_date_add_sub: {
            ...
            InitExprAtLogicalPlan(date_node->lnext, date_node->actual_type,
                                  colindex, schema);
            InitExprAtLogicalPlan(date_node->rnext, date_node->rnext_type,
                                  colindex, schema);  // the difference between
                                                      // t_qexpr_date_add_sub and t_qexpr_cal
            date_node->isnull = (date_node->lnext->isnull
                    || date_node->rnext->isnull);
            date_node->length = max(date_node->lnext->length,
                                    date_node->rnext->length);
        }
            break;
        case t_qexpr_func: {
...
void InitExprAtPhysicalPlan(QNode *node) {
    ...
    switch (node->type) {
        case t_qexpr_cal:  // binary calculation node
        {
            ...
            calnode->function_call = ExectorFunction::operator_function[calnode
                    ->actual_type][calnode->op_type];
            calnode->type_cast_func = TypeCast::type_cast_func[calnode
                    ->actual_type][calnode->return_type];
        ...
        case t_qexpr_cmp:  // binary comparison node
        {
            ...
            cmpnode->function_call = ExectorFunction::operator_function[cmpnode
                    ->actual_type][cmpnode->op_type];
            cmpnode->actual_type = t_boolean;  //
            cmpnode->type_cast_func = TypeCast::type_cast_func[cmpnode
                    ->actual_type][cmpnode->return_type];
        ...
        case t_qexpr_unary: {
            ...
            unode->function_call = ExectorFunction::operator_function[unode
                    ->actual_type][unode->op_type];
            unode->type_cast_func =
                    TypeCast::type_cast_func[unode->actual_type][unode
                            ->return_type];
        ...
        case t_qexpr_ternary: {
            ...
            tnode->function_call = ExectorFunction::operator_function[tnode
                    ->actual_type][tnode->op_type];
            tnode->type_cast_func =
                    TypeCast::type_cast_func[tnode->actual_type][tnode
                            ->return_type];
        ...
        case t_qexpr_case_when: {
            ...
            cwnode->type_cast_func = TypeCast::type_cast_func[cwnode
                    ->actual_type][cwnode->return_type];
        ...
        case t_qexpr_in: {
            ...
            innode->type_cast_func = TypeCast::type_cast_func[innode
                    ->actual_type][innode->return_type];
        ...
        case t_qexpr_date_add_sub: {
            ...
            date_node->function_call =
                    ExectorFunction::operator_function[date_node->actual_type][date_node
                            ->op_type];
            date_node->type_cast_func = TypeCast::type_cast_func[date_node
                    ->actual_type][date_node->return_type];
        ...
        case t_qcolcumns: {
            ...
            qcol->FuncId = getcol;
            qcol->type_cast_func =
                    TypeCast::type_cast_func[qcol->actual_type][qcol
                            ->return_type];
                       ...
                       // data type has casted
        {
            ...
            strcpy((char *) qexpr->value, qexpr->const_value.c_str());  // change the storage style from
                                                                        // string to char *,so store the
                                                                        // value in the return_type[]
            TypeCast::type_cast_func[t_string][qexpr->return_type](
                    qexpr->value, qexpr->value);
        }
            break;
        default: {
        }
