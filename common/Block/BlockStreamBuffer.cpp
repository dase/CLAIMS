...
BlockStreamBuffer::BlockStreamBuffer(unsigned block_size, unsigned block_count,
                                     Schema* schema)
    : received_block_count_(0),
      total_block_count_(block_count),
      block_size_(block_size) {
  lock_.acquire();
  for (unsigned i = 0; i < block_count; i++) {
    block_stream_empty_list_.push_back(
        BlockStreamBase::createBlock(schema, block_size));
...
bool BlockStreamBuffer::getBlock(BlockStreamBase& block) {
  ...
  if (!block_stream_used_list_.empty()) {
    ...
    sema_empty_block_.post();
    //		assert(block_stream_empty_list_.size()==sema_empty_block_.get_value());
    //		printf("[after get(suc)]:sema=%d,
    // empty_list=%d\n",sema_empty_block_.get_value(),block_stream_empty_list_.size());
    lock_.release();
    return true;
  }
  //	printf("[after get(fail)]sema=%d,
  // empty_list=%d\n",sema_empty_block_.get_value(),block_stream_empty_list_.size());
  lock_.release();
  return false;
}
