 ...
 * In fact, the swap of pointer is used rather than actually coping the content
 * for
 * performance concern.
 */
bool PartitionedBlockBuffer::insertBlockToPartitionedList(
    Block* src, unsigned partition_id, bool need_copy = false) {
  lock.acquire();
  inserted++;
  lock.release();
  ...
  if (need_copy) {
    ...
    // just switch content for avoiding memcpy()
    emptyBlock->SwitchBlock(src);
  }
...
unsigned PartitionedBlockBuffer::getBlockNumberOfPartitionInBuffer(
...
bool PartitionedBlockBuffer::getBlockInPartitionedBlockList(
    Block& desc, unsigned partition_id) {
  ...
  targetBlock->SwitchBlock(&desc);
  /*
   * The three lines above are replaced by the following lines for memory leak
   * debugging
   */
