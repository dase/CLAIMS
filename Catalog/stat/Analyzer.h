...
private:
	...
	static bool isBeneficalFromHistrogram(unsigned distinct_values,unsigned cardinality);
	/**
	 * get histogram if any, otherwise compute it.
	 */
	static Histogram* computeHistogram(const AttributeID& attr,const unsigned nbuckets=20);

	static unsigned decideNumberOfBucketsForHistogram(const int distinct_cardinality, const int cardinality);
};
