...
double ratio(const void *sourLow, const void *sourUp, const void *paraLow,
	...
	if (type->operate->compare(low, up) < 0) {
		...
		case t_u_long:
			...
			break;
		default:
			ret = 0.5; //For data type like String, the intersect ration computation does not exist.
			//Hence, we use the magic number to guess the ratio of values fallen into the both intervals.
			break;
		}
	}
	delete low;
...
unsigned long Estimation::estEqualJoin(AttributeID attrID1,
	...
	Histogram *hist1 = StatManager::getInstance()->getAttributeStatistic(
			attrID1)->getHistogram();
	Histogram *hist2 = StatManager::getInstance()->getAttributeStatistic(
			attrID2)->getHistogram();
	...
	if (attr1.attrType->type == attr2.attrType->type) {
		...
		for (int i = 0; i < hist1->m_bucketCnt - 1; ++i) {
			for (int j = 0; j < hist2->m_bucketCnt - 1; ++j) {
				ret += product(hist1->m_staValues1[i],
						hist1->m_staValues1[i + 1], hist1->m_staNumbers1[i],
						aveTupleNum1, hist2->m_staValues1[j],
						hist2->m_staValues1[j + 1], hist2->m_staNumbers1[j],
						aveTupleNum2, attr1.attrType);
			}
		}
		return ret;
	} else {
