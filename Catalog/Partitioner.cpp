...
Partitioner::~Partitioner() {
  ...
  for (auto iter = partition_info_list.begin();
...
bool Partitioner::bindPartitionToNode(PartitionOffset partition_id,
  ...
  //	printf("PartitionID:%d is bound to
...
unsigned long Partitioner::getPartitionCardinality(
  ...
  //	unsigned
  // tuple_bytes=Catalog::getInstance()->getProjection(projection_id_)->getSchema()->getTupleMaxSize();
...
unsigned Partitioner::getPartitionChunks(unsigned partition_index) const {
  return ceil(partition_info_list[partition_index]->number_of_blocks /
              (float)1024);
}
NodeID Partitioner::getPartitionLocation(unsigned partition_offset) const {
  if (partition_info_list[partition_offset]->get_mode() == OneToOne) {
    //		return
    //(*(OneToOnePartitionInfo*)&partition_info_list[partition_index]).get_location();
...
bool Partitioner::allPartitionBound() const {
  if (NULL != this) {
    ...
    LOG(WARNING) << "no partition has been bound to the nodes" << endl;
    return false;
  }
  return true;
}
