...
class ProjectionDescriptor {
 public:
  ...
  inline void setProjectionID(const ProjectionID& pid) {
    projection_id_ = pid;
  }
  inline map<string, set<string> > getFileLocations() const {
    return fileLocations;
  }
  inline ProjectionID getProjectionID() const {
...
//  void addProjection(vector<ColumnOffset> id_list);
  ...
  inline string get_table_name() const {
    return tableName;
  }
  inline TableID get_table_id() const {
    return table_id_;
  }
  ...
  inline unsigned long getRowNumber() {
    return row_number_;
  }
  inline unsigned long isEmpty() {
    return row_number_ == 0;
  }
