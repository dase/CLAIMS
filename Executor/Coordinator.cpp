...
bool Coordinator::SetupTheTheron(){
	...
	endpoint=new Theron::EndPoint((ip+":"+port.str()).c_str(),ip_port.c_str());
...
void* Coordinator::ListeningNewNode(void *arg){
	...
	while(true){
		...
		Cthis->logging->log("[Coordinator]: I'm waiting for the new node's connection!");
		...
		Cthis->logging->log("[Coordinator]: New Node is connected! Waiting for the port infor..");
		...
		Cthis->logging->log("[Coordinator]: The EndPoint of the new node is %s:%d",inet_ntoa(remote_addr.sin_addr),port);
		...
		if(!Cthis->endpoint->Connect(("tcp://"+new_node_ip+":"+new_node_port).c_str())){
			Cthis->logging->elog("Error occurs when the Coordinator EndPoint is connecting to the EndPoint of the new node:");
			Cthis->logging->log(" tcp://%s:%s",new_node_ip.c_str(),new_node_port.c_str());
		}

		Cthis->logging->log("[Coordinator]: The Coordinator EndPoint has successfully connected to the EndPoint of the new node!");
		...
		Cthis->logging->log("[Coordinator]: New node %s;%s is successfully added to the Theron communication network!",new_node_ip.c_str(),new_node_port.c_str());
...
bool Coordinator::SendReadyNotificationToNewNode(int socket_new_node){
	...
	if(!send(socket_new_node,&signal,sizeof(char),0)){
		logging->log("Error occurs when sending the Coordinate EndPoint port to the new node!");
	}
...
bool Coordinator::SendCoordinatorEndPointPort(int socket_new_node){
	...
	if(!send(socket_new_node,&port,sizeof(int),0)){
		logging->elog("Error occurs when sending the Coordinate EndPoint port to the new node!");
	}

	logging->log("[Coordinator]: The Coordinator EndPoint port [%d] is sent to the new node!",port);
