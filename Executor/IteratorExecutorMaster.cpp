...
bool IteratorExecutorMaster::ExecuteBlockStreamIteratorsOnSites(
  ...
  for (unsigned slave_filter_id = 0; slave_filter_id < ip_list.size();
       slave_filter_id++) {
    ...
    framework->Send(str, receiver.GetAddress(),
                    Theron::Address(ip_port.str().c_str()));
    logging_->log("send!\n********** please rebounce!\n");
  }
...
bool IteratorExecutorMaster::ExecuteBlockStreamIteratorsOnSite(
  ...
  //&Theron::Catcher<Message256>::Push);
  ...
  // t.tv_usec);
  framework->Send(str, Theron::Address(),
                  Theron::Address(actor_name.str().c_str()));
  ...
  //		return true;
  //	}
  //	logging_->log("Received the confirm feedback from
  //%s",ip_port.str().c_str());
  return true;
#else
  PhysicalQueryPlan* im = new PhysicalQueryPlan(it);
...
bool IteratorExecutorMaster::Propogation(const int count, std::string target) {
  ...
  framework->Send(count, Theron::Address(),
                  Theron::Address(ip_port.str().c_str()));
  sleep(100);
}
