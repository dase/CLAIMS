...
filter_process_func getFilterProcessFunc(QNode* qnode, Schema* schema) {
  /*
   *    int process_func(char* b_start, int & b_cur_addr, int b_tuple_count,
  ...
  llvm::Function* F =
      llvm::Function::Create(FT, llvm::Function::ExternalLinkage, "a",
                             CodeGenerator::getInstance()->getModule());

  /* create function entry */
  llvm::BasicBlock* entry_block =
      llvm::BasicBlock::Create(llvm::getGlobalContext(), "entry_block", F);
  llvm::BasicBlock* while_cond =
      llvm::BasicBlock::Create(llvm::getGlobalContext(), "while_cond", F);
  llvm::BasicBlock* while_body =
      llvm::BasicBlock::Create(llvm::getGlobalContext(), "while_body", F);
  llvm::BasicBlock* while_end =
      llvm::BasicBlock::Create(llvm::getGlobalContext(), "while_end", F);
  llvm::BasicBlock* if_passed =
      llvm::BasicBlock::Create(llvm::getGlobalContext(), "if_passed", F);
  llvm::BasicBlock* if_has_space =
      llvm::BasicBlock::Create(llvm::getGlobalContext(), "if_has_space", F);
  llvm::BasicBlock* exit_block =
      llvm::BasicBlock::Create(context, "exit_block", F);
  ...
  llvm::Value* const_int_32_1 =
      llvm::ConstantInt::get(llvm::Type::getInt32Ty(context), 1);
  llvm::Value* const_int_32_3 =
  ...
  // where body
  {
    ...
    llvm::Value* pass_addr =
        builder->CreateAlloca(llvm::Type::getInt1Ty(context));
    c_cur = builder->CreateLoad(c_cur_addr);
    llvm::Value* c_offset = builder->CreateMul(c_cur, tuple_size);
    llvm::Value* c_offset_64 =
        builder->CreateIntCast(c_offset, llvm::Type::getInt64Ty(context), true);
    llvm::Value* c_start_64 =
        builder->CreatePtrToInt(c_start, llvm::Type::getInt64Ty(context));
    ...
    // if body
    {
      ...
      // if body
      {
        ...
        llvm::Value* b_start_64 =
            builder->CreatePtrToInt(b_start, llvm::Type::getInt64Ty(context));
...
llvm::Function* getExprLLVMFucn(QNode* qnode, Schema* schema) {
  ...
  std::vector<llvm::Type*> parameter_types;
  parameter_types.push_back(llvm::PointerType::getUnqual(
...
expr_func getExprFunc(QNode* qnode, Schema* schema) {
  ...
  if (fun == 0) {
    ...
    expr_func ret = (expr_func)CodeGenerator::getInstance()
                        ->getExecutionEngine()
...
ExprFuncTwoTuples getExprFuncTwoTuples(QNode* qnode, Schema* l_schema,
  ...
  if (fun == 0) {
    ...
    ExprFuncTwoTuples ret = (ExprFuncTwoTuples)CodeGenerator::getInstance()
                                ->getExecutionEngine()
                                ->getPointerToFunction(fun);
    CodeGenerator::getInstance()->release();
    return ret;
...
llvm::Value* codegen(QNode* qnode, Schema* schema, llvm::Value* tuple_addr,
  ...
  switch (qnode->type) {
    case t_qexpr_cmp: {
      ...
      llvm::Value* lvalue =
          codegen(node->lnext, schema, tuple_addr, r_schema, r_tuple_addr);
      llvm::Value* rvalue =
          codegen(node->rnext, schema, tuple_addr, r_schema, r_tuple_addr);
      value = codegen_binary_op(lvalue, rvalue, node);
      actual_type = t_boolean;  // for boolean compression, the actual type is
                                // always boolean.
      break;
    }
    case t_qexpr_cal: {
...
llvm::Value* codegen_column(QColcumns* node, Schema* schema,
  ...
  llvm::Value* value_off = ConstantInt::get(
      llvm::Type::getInt64Ty(llvm::getGlobalContext()), (int64_t)offset, false);
  ...
  switch (node->actual_type) {
    case t_int:
      ...
      column_addr = builder->CreateIntToPtr(
          column_addr, llvm::PointerType::getUnqual(
  ...
  switch (node->return_type) {
    case t_int:
      ...
      dest_ptr = builder->CreatePointerCast(
          dest_ptr, llvm::PointerType::getUnqual(
                        llvm::Type::getInt32Ty(llvm::getGlobalContext())));
      ...
      switch (target_ty) {
        ...
        case t_u_long:
          return builder->CreateIntCast(
              v, llvm::Type::getInt64Ty(llvm::getGlobalContext()), true,
              "long");
        case t_double:
          return builder->CreateSIToFP(
...
llvm::Function* getExprLLVMFuncForTwoTuples(QNode* qnode, Schema* l_schema,
  ...
  llvm::Value* return_value =
      codegen(qnode, l_schema, l_tuple_addr, r_schema, r_tuple_addr);
...
QNode* createEqualJoinExpression(Schema* l_s, Schema* r_s,
  ...
  for (int i = 0; i < l_join_index.size(); i++) {
    QColcumns* l_column =
        new QColcumns("L", "L.A", l_s->getcolumn(l_join_index[i]).type, "L.A");
    l_column->id = l_join_index[i];
    l_column->tab_index = 0;
    QColcumns* r_column =
        new QColcumns("R", "R.A", r_s->getcolumn(r_join_index[i]).type, "R.A");
    r_column->id = r_join_index[i];
    r_column->tab_index = 1;
    data_type return_type = TypePromotion::arith_type_promotion_map
        [l_s->getcolumn(l_join_index[i]).type][r_s->getcolumn(r_join_index[i])
                                                   .type];
...
LLVMMemcat getMemcat(unsigned length1, unsigned length2) {
  ...
  desc = builder->CreateIntToPtr(
      desc, llvm::PointerType::getUnqual(
                llvm::Type::getInt8Ty(llvm::getGlobalContext())));
...
// ConstantInt* const10 = ConstantInt::get(mod->getContext(), APInt(32,
// StringRef("1"), 10));
//
...
// ptr_a->setName("a");
//
// BasicBlock* label_entry = BasicBlock::Create(mod->getContext(),
// "entry",func__Z4fuckRi,0);
//
...
void test_reference() {
  ...
  ConstantInt* const10 =
      ConstantInt::get(llvm::getGlobalContext(), APInt(32, StringRef("1"), 10));

  BasicBlock* label_entry =
      BasicBlock::Create(llvm::getGlobalContext(), "entry", F, 0);
  ...
  llvm::AllocaInst* ptr_a_addr =
      builder->CreateAlloca(llvm::PointerType::getUnqual(
          llvm::Type::getInt32Ty(llvm::getGlobalContext())));
  ...
  func ret = (func)CodeGenerator::getInstance()
                 ->getExecutionEngine()
                 ->getPointerToFunction(F);
  int a = 1024;
  ret(a);
...
void test_while() {
  ...
  //    llvm::Function::ExternalLinkage, "a",
  //    CodeGenerator::getInstance()->getModule());
  //
  //
  ...
  //    a->setName("a");
  //
  //    llvm::Value* tmp=ConstantInt::get(llvm::getGlobalContext(), APInt(32,
  //    StringRef("2"), 10));
  //
  //
  //
...
void myllvm::test() {
  ...
  // Function: _Z4funci (func__Z4funci)
  {
    ...
    BasicBlock* label_entry =
        BasicBlock::Create(mod->getContext(), "entry", func__Z4funci, 0);
    BasicBlock* label_while_cond =
        BasicBlock::Create(mod->getContext(), "while.cond", func__Z4funci, 0);
    BasicBlock* label_while_body =
        BasicBlock::Create(mod->getContext(), "while.body", func__Z4funci, 0);
    BasicBlock* label_while_end =
        BasicBlock::Create(mod->getContext(), "while.end", func__Z4funci, 0);
    ...
    BinaryOperator* int32_mul = BinaryOperator::Create(
        Instruction::Mul, int32_10, int32_11, "mul", label_while_body);
    StoreInst* void_12 =
        new StoreInst(int32_mul, ptr_b, false, label_while_body);
...
void myllvm::test1() {
  ...
  // Function: _Z4funci (func__Z4funci)
  {
    ...
    AllocaInst* ptr_c_addr =
        builder->CreateAlloca(IntegerType::get(mod->getContext(), 32));
    ptr_c_addr->setAlignment(4);
    AllocaInst* ptr_b =
        builder->CreateAlloca(IntegerType::get(mod->getContext(), 32));
    ptr_b->setAlignment(4);
