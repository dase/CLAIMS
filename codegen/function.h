...
inline ExprAST *Error(const char *Str) {
  fprintf(stderr, "Error: %s\n", Str);
  return 0;
}
inline PrototypeAST *ErrorP(const char *Str) {
  Error(Str);
  return 0;
}
inline FunctionAST *ErrorF(const char *Str) {
  Error(Str);
  return 0;
}
...
inline std::string GenerateUniqueName(const char *root) {
  static int i = 0;
  char s[16];
...
inline std::string MakeLegalFunctionName(std::string Name) {
  std::string NewName;
  if (!Name.length())
...
extern "C" inline double putchard(double X) {
  putchar((char)X);
  return 0;
}
