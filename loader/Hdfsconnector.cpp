      ...
      switch (open_flag_) {
        ...
        case APPENDD: {
          if (hdfsExists(fs, (*par_writepath).c_str()) == -1) {
            ...
            break;
            //					cout << "[ERROR:
            //Hdfsconnector.cpp->assgin_open_file()]:
...
bool HdfsConnector::OpenFiles(open_flag open_flag_) {
  ...
  if (file_handles_.size() == 0) {
    cout << "[ERROR: Hdfsconnector.cpp->op_connect()]: the writefile is empty!"
         << endl;
    exit(-1);
  } else {
...
int HdfsConnector::Flush(unsigned projection_offset, unsigned partition_offset,
                         const void* source, unsigned length) {
  return hdfsWrite(fs, file_handles_[projection_offset][partition_offset],
                   source, length);
}
