...
using namespace std;
LocalDiskConnector::LocalDiskConnector(vector<vector<string> > hdfs_writepath)
...
bool LocalDiskConnector::OpenFiles(open_flag open_flag_) {
  ...
  for (prj_writepath = writepath.begin(); prj_writepath != writepath.end();
    ...
    for (par_writepath = (*prj_writepath).begin();
      ...
      switch (open_flag_) {
        case CREATEE: {
          int fd = FileOpen((*par_writepath).c_str(),
                            O_RDWR | O_TRUNC | O_CREAT, S_IWUSR | S_IRUSR);
          if (fd == -1) {
            PLOG(ERROR) << "Cannot create disk file! Reason:";
          }
        ...
        default: {
          LOG(ERROR)
              << "[ERROR: Hdfsconnector.cpp->assgin_open_file()]: Illegal "
...
int LocalDiskConnector::Flush(unsigned projection_offset,
                              ...
                              unsigned length) {
  return write(file_handles_[projection_offset][partition_offset], source,
               length);
}
